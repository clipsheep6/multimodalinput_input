...
public:
	...
	*@param filter Indicates the input event filter to set. When an input event occurs, this filter is called and returns a value indicating whether to continue processing the input event.
	*		If the filter returns <b>true</b>, the processing of the input event ends. If the filter returns <b>false</b>, the processing of the input event continues.
	*@since 8
	*/
	...
	*@param inputEventConsumer Indicates the consumer to set. The window input event of the current process will be called back to the consumer object for processing.
	*@since 8
	*/
