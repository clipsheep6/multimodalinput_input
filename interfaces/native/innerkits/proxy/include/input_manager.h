...
public:
	...
	*@param filter Indicates the input event filter to set. When an input event occurs, this filter is called 
    *and returns a value indicating whether to continue processing the input event.If the filter returns <b>true</b>, 
    *the processing of the input event ends. If the filter returns <b>false</b>, the processing of the input event continues.
	*@since 8
	*/
	void SetInputEventFilter(std::function<bool(std::shared_ptr<PointerEvent> filter)>); 	
