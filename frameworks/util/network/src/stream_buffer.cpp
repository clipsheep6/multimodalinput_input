...
{
    if (rIdx_ == wIdx_) {
        MMI_LOG_E("Not enough memory to read... errCode:%{public}d", MEM_NOT_ENOUGH);
        return false;
    }
    buf = ReadBuf();
...
bool OHOS::MMI::StreamBuffer::Read(char *buf, size_t size)
{
    ...
    if (rIdx_ + size > wIdx_) {
        MMI_LOG_E("Memory out of bounds on read... errCode:%{public}d", MEM_OUT_OF_BOUNDS);
        return false;
    }
...
bool OHOS::MMI::StreamBuffer::Write(const char *buf, size_t size)
{
    ...
    if (wIdx_ + size >= MAX_STREAM_BUF_SIZE) {
        MMI_LOG_E("Memory out of bounds on write... errCode:%{public}d", MEM_OUT_OF_BOUNDS);
        return false;
    }
