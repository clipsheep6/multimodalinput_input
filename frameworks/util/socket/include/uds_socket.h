...
public:
    ...
    virtual int32_t Socket(int32_t domain, int32_t type);
    virtual int32_t Close();
    virtual int32_t Bind(const std::string& path = "");
    virtual int32_t Connect(const std::string& path);
    virtual int32_t Listen(int32_t n);
    virtual int32_t Accept(struct sockaddr_un& addr);
    ...
    std::string GetBindPath() const
    {
        return bindPath_;
    }
    std::string GetConnPath() const
    {
        return connPath_;
    }
