...
size_t UDSSocket::Read(char *buf, size_t size)
{
    ...
    size_t ret = read(fd_, static_cast<void *>(buf), size);
    if (ret < 0) {
        MMI_LOGE("UDSSocket::Read read return %{public}zu", ret);
    }
    return ret;
}
...
size_t UDSSocket::Write(const char *buf, size_t size)
{
    ...
    if (ret < 0) {
        MMI_LOGE("UDSSocket::Write write return %{public}zu", ret);
    }
    return ret;
}
...
size_t UDSSocket::Send(const char *buf, size_t size, int32_t flags)
{
    CHKPR(buf, -1);
    CHKR(size > 0, PARAM_INPUT_INVALID, -1);
    size_t ret = send(fd_, buf, size, flags);
    if (ret < 0) {
        MMI_LOGE("UDSSocket::Send send return %{public}zu", ret);
    }
    return ret;
}
...
size_t UDSSocket::Recv(char *buf, size_t size, int32_t flags)
{
    CHKPR(buf, -1);
    CHKR(size > 0, PARAM_INPUT_INVALID, -1);
    size_t ret = recv(fd_, static_cast<void *>(buf), size, flags);
    if (ret < 0) {
        MMI_LOGE("UDSSocket::Recv recv return %{public}zu", ret);
    }
    return ret;
}
...
size_t UDSSocket::Recvfrom(char *buf, size_t size, uint32_t flags, sockaddr *addr, size_t *addrlen)
{
    ...
    CHKR(size > 0, PARAM_INPUT_INVALID, -1);
    CHKR(fd_ >= 0, PARAM_INPUT_INVALID, -1);
    size_t ret = recvfrom(fd_, static_cast<void *>(buf), size, flags, addr, reinterpret_cast<socklen_t *>(addrlen));
    if (ret < 0) {
        MMI_LOGE("UDSSocket::Recvfrom recvfrom return %{public}zu", ret);
    }
    return ret;
}
...
size_t UDSSocket::Sendto(const char *buf, size_t size, uint32_t flags, sockaddr *addr, size_t addrlen)
{
    ...
    CHKR(size > 0, PARAM_INPUT_INVALID, -1);
    CHKR(fd_ >= 0, PARAM_INPUT_INVALID, -1);
    size_t ret = sendto(fd_, static_cast<const void *>(buf), size, flags, addr, static_cast<socklen_t>(addrlen));
    if (ret < 0) {
        MMI_LOGE("UDSSocket::Sendto sendto return %{public}zu", ret);
    }
    return ret;
}
