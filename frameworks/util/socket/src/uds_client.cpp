...
bool OHOS::MMI::UDSClient::SendMsg(const char *buf, size_t size) const
{
    ...
    if (ret < 0) {
        MMI_LOGE("SendMsg write errCode:%{public}d return %{public}ld", MSG_SEND_FAIL, ret);
        return false;
    }
    return true;
}
...
{
    if (!isThreadHadRun_) {
        MMI_LOGI("thread is not run. this: %p, isThreadHadRun_: %p, isThreadHadRun_: %d", this, &isThreadHadRun_, isThreadHadRun_);
