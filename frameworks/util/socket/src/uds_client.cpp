...
int32_t OHOS::MMI::UDSClient::ConnectTo(const std::string& path, bool isBind)
{
    CHKR(!path.empty(), PARAM_INPUT_INVALID, RET_ERR);
    CHKR(Socket(AF_LOCAL, SOCK_STREAM) >= 0, SOCKET_CREATE_FAIL, RET_ERR);
    if (isBind) {
        CHKR(Bind() >= 0, SOCKET_BIND_FAIL, RET_ERR);
    }
    isBind_ = isBind;
    CHKR(Connect(path) == RET_OK, CONN_FAIL, RET_ERR);
...
bool OHOS::MMI::UDSClient::SendMsg(const char *buf, size_t size) const
{
    ...
    auto ret = write(fd_, static_cast<const void *>(buf), size);
    if (ret < 0) {
        MMI_LOGE("SendMsg write errCode:%{public}d return %{public}d", MSG_SEND_FAIL, ret);
        return false;
    }
    return true;
}
...
bool OHOS::MMI::UDSClient::StartClient(MsgClientFunCallback fun)
{
    recvFun_ = fun;
    isRun_ = true;
    isConnected_ = true;
    if (ConnectTo(connPath_, isBind_) < 0) {
        MMI_LOGW("Client connection failed...Try again later...");
        isConnected_ = false;
#ifndef ENABLE_CLIENT_RECONNECT
        return false;
#endif
    }
...
void OHOS::MMI::UDSClient::OnThread()
{
    ...
    while (isRun_) {
        if (isConnected_) {
            streamBuf.Clean();
            auto count = EpollWait(*events, MAX_EVENT_SIZE, -1);
            for (auto i = 0; i < count; i++) {
                OnEvent(events[i], streamBuf);
            }
        } else {
            if (ConnectTo(connPath_.c_str(), isBind_) < 0) {
