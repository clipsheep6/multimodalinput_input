...
bool OHOS::MMI::SafeKeeper::RegisterEvent(uint64_t tid, const std::string& remark)
{
    ...
    MMI_LOGI("SafeKeeper register tid:[%{public}zu] remark:[%{public}s]", tid, remark.c_str());
    return true;
}
...
void OHOS::MMI::SafeKeeper::ReportHealthStatus(uint64_t tid)
{
    ...
    if (!ptr) {
        MMI_LOGE("SafeKeeper report ptr = nullptr tid:[%{public}zu] errCode:%{public}d", tid, NULL_POINTER);
        return;
    }
