...
uint64_t GetSysClockTime()
{
    ...
    return (ts.tv_sec * static_cast<uint64_t>(1e6)) + (ts.tv_nsec / 1000);
}
...
std::string Strftime(const std::string &format, time_t curTime)
{
    ...
    LocalTime(t, curTime);
    char szDTime[32] = "";
    strftime(szDTime, sizeof(szDTime), format.c_str(), &t);
    return szDTime;
}
    ...
    const int32_t abilityId, const int32_t focusId, const uint64_t preHandlerTime)
{
    MMI_LOGT("4.event dispatcher of server: EventJoyStickAxis:deviceId: %{public}u; devicePhys: %{public}s;"
             ...
             "time: %{public}zu; deviceType: %{public}u; eventType: %{public}d; deviceName: %{public}s\n",
...
char* mmi_basename(char* path)
{
	...
	char* p_basename = right_slash ? (right_slash + 1) : path;
