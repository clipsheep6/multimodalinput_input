...
void LogManager::ParseLogLimitSize(const std::string& str)
{
    ...
    if (LOG_MAX_FILE_SIZE != size && size != limitSize_) {
        LOGLOG("The log LimitSize will be modified. The old is %llu and the new is %llu",
               size, limitSize_);
    }
}
...
bool LogManager::Start(void)
{
    ...
    SemCreate(0);
    t_ = std::thread(std::bind(&LogManager::Run, this));
...
const std::vector<char> LogManager::RemovePrivacyIdentifierInFmt(const std::string& format)
{
    ...
    while (readPos < formatLen) {
        if (formatLen - readPos >= (oneLineSize - 1)) {
            ...
            if (f(0) == '%' && f(1) == '{' && f(2) == 'p' && f(3) == 'u' && f(4) == 'b' && 
                f(5) == 'l' && f(6) == 'i' && f(7) == 'c' && f(8) == '}') {
        ...
        if (formatLen - readPos >= oneLineSize) {
            ...
            if (f(0) == '%' && f(1) == '{' && f(2) == 'p' && f(3) == 'r' && f(4) == 'i' && 
                f(5) == 'v' && f(6) == 'a' && f(7) == 't' && f(8) == 'e' && f(9) == '}') {
                readPos += oneLineSize;
                format2.push_back('%');
                ++writePos;
                continue;
