...
// LogManager
void mmi_console_log(bool withoutFileInfo, char* fileName, int lineNo, int level, const std::string& threadName, const char* fmt, ...);
...
#define LOGLOG(fmt, ...) do { \
    mmi_console_log(false, (char *)__FILE__, __LINE__, OHOS::MMI::LL_INFO, OHOS::MMI::GetThreadName(), MMI_FUNC_FMT fmt "[only_console]", MMI_FUNC_INFO, ##__VA_ARGS__);\
...
const std::vector<char> RemovePrivacyIdentifierInFmt(const std::string& format)
{
	...
	while (readPos < formatLen) {
		if (formatLen - readPos >= (oneLineSize - 1)) {
			auto f = [&format, &readPos](size_t index) {
				return format.at(readPos + index);
			};
			if (f(0) == '%' && f(1) == '{' && f(2) == 'p' && f(3) == 'u' && f(4) == 'b' && f(5) == 'l' && f(6) == 'i' &&
				f(7) == 'c' && f(8) == '}') {
		...
		if (formatLen - readPos >= oneLineSize) {
			...
			if (f(0) == '%' && f(1) == '{' && f(2) == 'p' && f(3) == 'r' && f(4) == 'i' && f(5) == 'v' && f(6) == 'a' &&
				f(7) == 't' && f(8) == 'e' && f(9) == '}') {
				readPos += oneLineSize;
				format2.push_back('%');
				++writePos;
...
#pragma GCC diagnostic pop
    ...
    printf("%s%s [%s] [%s] [%s] [%s] %s ", LOG_COLORS[level].c_str(), longTime, GetProcessId().c_str(), GetThreadId().c_str(),
...
{
    if (!pLog) {
        LOGLOG("*** FATAL ERROR ***: in %s, #%d, pLog is nullptr.\n", __func__, __LINE__);
        return;
    }
    // 时间
    char longTime[LOG_MAX_TIME_LEN] = {};
    ...
    if (ret < 0) {
        LOGLOG("*** FATAL ERROR ***: in %s, #%d, call snprintf_s fail.\n", __func__, __LINE__);
        return;
    }
    ...
    // 控制台输出
    if (logDisplay_) {
        MMI_LOG_MANAGER_OUT("%s%s [%s] [%s] [%s] [%s] %s ", LOG_COLORS[pLog->level].c_str(), longTime, proId_.c_str(), pLog->threadId_.c_str(),
...
bool LogManager::Start(void)
{
    ...
    SemCreate(0);
    t_ = std::thread(std::bind(&LogManager::OnThread, this, std::ref(threadPromiseHadRunning_), std::ref(threadPromiseHadEnd_)));
...
{
    if (runing_) {
        LOGLOG("LogManager::stop, log thread stoping, log queue size: %zd.", logs_.size());
		toExit_ = true;
		//runing_ = false;
        // while (!logs_.empty()) {
            // logs_.pop();
        // }        
        Wait();
        LOGLOG("LogManager::stop, wait log thread stoping end, log queue size: %zd.", logs_.size());
        return true;
    }

    LOGLOG("LogManager::stop, log had stoped., log queue size: %zd.", logs_.size());
    return false;
}
