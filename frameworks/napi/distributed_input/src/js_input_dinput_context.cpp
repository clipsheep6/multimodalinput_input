...
namespace MMI {
namespace {
constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, MMI_LOG_DOMAIN, "JsInputDinputContext" }; // namespace
...
JsInputDinputContext::JsInputDinputContext()
{
    mager_ = std::make_shared<JsInputDinputManager>();
    CHKPL(mager_);
}
...
napi_value JsInputDinputContext::CreateInstance(napi_env env)
{
    ...
    napi_status status = napi_define_class(env, className, sizeof(className), JsInputDinputContext::JsConstructor, nullptr,
...
napi_value JsInputDinputContext::JsConstructor(napi_env env, napi_callback_info info)
{
    ...
    napi_status status = napi_wrap(env, thisVar, jsContext, [](napi_env env, void* data, void* hin) {
        ...
        JsInputDinputContext *context = (JsInputDinputContext*)data;
        delete context;
    }, nullptr, nullptr);
...
std::shared_ptr<JsInputDinputManager> JsInputDinputContext::GetJsInputDinputMgr()
{
    return mager_;
}
...
napi_value JsInputDinputContext::PrepareRemoteInput(napi_env env, napi_callback_info info)
{
    ...
    if (JsInputDinputContext::GetParameter(env, info, deviceId, handlerRef) != nullptr) {
        JsInputDinputContext *jsContext = JsInputDinputContext::GetInstance(env);
        auto jsInputDinputMgr = jsContext->GetJsInputDinputMgr();
        return jsInputDinputMgr->PrepareRemoteInput(env, deviceId, handlerRef);
    }
    return nullptr;
}
...
napi_value JsInputDinputContext::UnprepareRemoteInput(napi_env env, napi_callback_info info)
{
    ...
    if (JsInputDinputContext::GetParameter(env, info, deviceId, handlerRef) != nullptr) {
        JsInputDinputContext *jsContext = JsInputDinputContext::GetInstance(env);
        auto jsInputDinputMgr = jsContext->GetJsInputDinputMgr();
        return jsInputDinputMgr->UnprepareRemoteInput(env, deviceId, handlerRef);
    }
    return nullptr;
}
...
napi_value JsInputDinputContext::StartRemoteInput(napi_env env, napi_callback_info info)
{
    ...
    napi_ref handlerRef {nullptr};
    if (JsInputDinputContext::GetParameter(env, info, deviceId, inputAbility, handlerRef) != nullptr) {      
        JsInputDinputContext *jsContext = JsInputDinputContext::GetInstance(env);
        auto jsInputDinputMgr = jsContext->GetJsInputDinputMgr();
        return jsInputDinputMgr->StartRemoteInput(env, deviceId, inputAbility, handlerRef);
    }
    return nullptr;
}
...
napi_value JsInputDinputContext::StopRemoteInput(napi_env env, napi_callback_info info)
{
    ...
    if (JsInputDinputContext::GetParameter(env, info, deviceId, inputAbility, handlerRef) != nullptr) {
        JsInputDinputContext *jsContext = JsInputDinputContext::GetInstance(env);
        auto jsInputDinputMgr = jsContext->GetJsInputDinputMgr();
        return jsInputDinputMgr->StopRemoteInput(env, deviceId, inputAbility, handlerRef);
    }
    return nullptr;
}
...
napi_value JsInputDinputContext::GetRemoteInputAbility(napi_env env, napi_callback_info info)
{
    ...
    if (JsInputDinputContext::GetParameter(env, info, deviceId, handlerRef) != nullptr) {
        JsInputDinputContext *jsContext = JsInputDinputContext::GetInstance(env);
        auto jsInputDinputMgr = jsContext->GetJsInputDinputMgr();
        return jsInputDinputMgr->GetRemoteInputAbility(env, handlerRef, deviceId);
    }
    return nullptr;
}
...
napi_value JsInputDinputContext::GetParameter(napi_env env, napi_callback_info info, std::string& first, napi_ref& second)
{
    CALL_LOG_ENTER;
    ...
    size_t typeLen = 0;
    CHKRP(env, napi_get_value_string_utf8(env, argv[ARGV_FIRST], deviceId, MAX_STRING_LEN - 1, &typeLen), GET_STRING_UTF8);
    first = deviceId;
    if (argc == ARGC_NUM_1) {
        second = nullptr;
        return ret;
...
napi_value JsInputDinputContext::InitInputAbilityTypeEnum(napi_env env, napi_value exports)
{
    ...
    napi_create_uint32(env, static_cast<uint32_t>(InputAbilityType::TOUCH_PAD), &touchpad);


    napi_property_descriptor desc[] = {
        DECLARE_NAPI_STATIC_PROPERTY("MOUSE", mouse),
        DECLARE_NAPI_STATIC_PROPERTY("KEYBOARD", keyboard),
        DECLARE_NAPI_STATIC_PROPERTY("TOUCH_PAD", touchpad),
    };
