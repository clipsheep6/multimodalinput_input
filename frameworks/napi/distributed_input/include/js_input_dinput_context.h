...
public:
    ...
    static napi_value GetParameter(napi_env env, napi_callback_info info, std::string& first, napi_ref& second);
    static napi_value GetParameter(napi_env env, napi_callback_info info,int32_t& first, int32_t& second, napi_ref& third);
    static napi_value GetParameter(napi_env env, napi_callback_info info, std::string& first, int32_t second, napi_ref& third);
    static napi_value GetParameter(napi_env env, napi_callback_info info, std::string& first, std::vector<uint32_t>& second, napi_ref& third);
    static bool TypeOf(napi_env env, napi_value value, napi_valuetype type);
    std::shared_ptr<JsInputDinputManager> GetJsInputDinputMgr();
    napi_ref contextRef_ {nullptr};

private:
    std::shared_ptr<JsInputDinputManager> mager_ {nullptr};
    std::mutex mtx_;
};
} // namespace MMI
} // namespace OHOS
