...
int32_t JsInputConsumerUtil::UnsubscribeCallback(napi_env env,
    const std::shared_ptr<SubscribeInfo> &event, Callbacks &callbacks, int32_t &subscribeId)
{
    ...
    if (DelEventCallback(env, infos, handler1, subscribeId) != RET_OK) {
        MMI_HILOGE("Delete callback failed");
        return RET_ERR;
    }
...
int32_t JsInputConsumerUtil::DelEventCallback(napi_env env, std::list<std::shared_ptr<SubscribeInfo>> &infos,
