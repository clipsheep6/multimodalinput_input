...
napi_value JsShortKeyContext::SetKeyDownDuration(napi_env env, napi_callback_info info)
{
    ...
    if (argc < 2) {
        ...
        THROWERR_API9(env, COMMON_PARAMETER_ERROR, "key", "string");
        return nullptr;
    }
    if (!JsCommon::TypeOf(env, argv[0], napi_string)) {
        MMI_HILOGE("key parameter type is invalid");
        THROWERR_API9(env, COMMON_PARAMETER_ERROR, "key", "string");
        return nullptr;
    }
    // 校验参数key参数的合法性
    char abilityKey[MAX_STRING_LEN] = { 0 };
    size_t ret = 0;
    CHKRP(napi_get_value_string_utf8(env, argv[0], abilityKey, MAX_STRING_LEN - 1, &ret), GET_VALUE_STRING_UTF8);
    
    // 校验参数keyDownDuration的合法性
    int32_t keyDownDuration = 0;
    CHKRP(napi_get_value_int32(env, argv[1], &keyDownDuration), GET_VALUE_INT32);
    if (keyDownDuration < 0) {
        MMI_HILOGE("Invalid keyDownDuration");
        THROWERR_CUSTOM(env, COMMON_PARAMETER_ERROR, "keyDownDuration is invalid");
        return nullptr;
    }
    if (!JsCommon::TypeOf(env, argv[1], napi_number)) {
        MMI_HILOGE("keyDownDuration parameter type is invalid");
    ...
    if (argc == 2) {
        return jsShortKeyMgr->SetKeyDownDuration(env, abilityKey, keyDownDuration);
    }
    ...
    return jsShortKeyMgr->SetKeyDownDuration(env, abilityKey, keyDownDuration, argv[2]);
}
