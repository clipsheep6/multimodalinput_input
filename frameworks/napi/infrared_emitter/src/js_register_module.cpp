...
static napi_value TransmitInfrared(napi_env env, napi_callback_info info) {
    //MMI_HILOGI(" js_register_module.TransmitInfrared start para :  " );
    CALL_DEBUG_ENTER;
    napi_value result = nullptr;
    int64_t number =-1;
    ...
    /********处理权限错误的问题 *******/
    if(0 > ret) {
        MMI_HILOGE("js_register_module.TransmitInfrared  requst error. Permission Error. returnCode:%{public}d", ret);
        ThrowError(env, ret, "TransmitInfrared");
        return nullptr;    
    }         
    if(ret != RET_OK) {
        if(COMMON_PERMISSION_CHECK_ERROR == ret) {      
            MMI_HILOGE("js_register_module.TransmitInfrared  requst error. Permission Error. Positive returnCode:%{public}d", ret);      
            ThrowError(env, ret, "TransmitInfrared");
             return nullptr;   
        }
