...
void JsEventTarget::EmitAddedDeviceEvent(uv_work_t *work, int32_t status)
{
    ...
    for (const auto &item : addEvent->second) {
        ...
        CHKRV(item->env, napi_set_named_property(item->env, resultObj[1], "ChangedType",
            eventType), SET_NAMED_PROPERTY);
        CHKRV(item->env, napi_set_named_property(item->env, resultObj[1], "deviceId",
            deviceId), SET_NAMED_PROPERTY);
#else
