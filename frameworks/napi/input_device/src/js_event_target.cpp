...
void JsEventTarget::EmitAddedDeviceEvent(uv_work_t *work, int32_t status)
{
    ...
    auto workItem = static_cast<std::unique_ptr<JsUtil::CallbackInfo> *>(temp->data);
    JsUtil::DeletePtr<uv_work_t *>(work);
    ...
    for (const auto &item : addEvent->second) {
        ...
        CHKRV_SCOPE_DEL(item->env, napi_get_reference_value(item->env, item->ref, &handler), GET_REFERENCE_VALUE, scope, &temp);
