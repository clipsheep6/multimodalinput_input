...
JsInputDeviceContext::JsInputDeviceContext(napi_env env)
{
    jsInputDeviceMgr_ = std::make_shared<JsInputDeviceManager>(env);
...
napi_value JsInputDeviceContext::GetDeviceIds(napi_env env, napi_callback_info info)
{
    ...
    if (argc == 1) {
        ...
        jsInputDeviceMgr->GetDeviceIdsAsync(argv[0]);
    } else {
        // promise
    }
    MMI_LOGT("end");
...
napi_value JsInputDeviceContext::GetDevice(napi_env env, napi_callback_info info)
{
    ...
    if (argc == 1) {
        ...
        jsInputDeviceMgr->GetDeviceAsync(id, argv[1]);
    }
    return nullptr;
}
