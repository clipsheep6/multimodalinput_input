...
void ClientMsgHandler::Init()
{
    MsgCallback funs[] = {
        ...
        {MmiMessageId::ADD_INPUT_DEVICE_MONITOR, MsgCallbackBind2(&ClientMsgHandler::OnDevListener, this)},
...
{
    CALL_LOG_ENTER;
    int32_t userData, id, deviceType, bus, product, vendor, version;
    std::string name, phys, uniq;
    size_t size;
    pkt >> userData >> id >> name >> deviceType >> bus >> product >> vendor >> version >> phys >> uniq >> size;
    if (pkt.ChkRWError()) {
    ...
    for (size_t i = 0; i < size; ++i) {
        int32_t type, min, max, fuzz, flat, resolution;
        pkt >> type >> min >> max >> fuzz >> flat >> resolution;
        if (pkt.ChkRWError()) {
            MMI_HILOGE("Packet read axis data failed");
            return PACKET_READ_FAIL;
        }
        InputDevice::AxisInfo axis(type, min, max, fuzz, flat, resolution);
        axisInfo.push_back(axis);
    }
    auto devData =
        std::make_shared<InputDevice>(id, name, deviceType, bus, version, product, vendor, phys, uniq, axisInfo);
