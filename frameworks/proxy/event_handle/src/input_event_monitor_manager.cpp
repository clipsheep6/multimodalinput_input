...
{
    MMI_LOG_T("InputEventMonitorManager::InputEventMonitorManager enter");
}
    ...
    std::function<void (std::shared_ptr<OHOS::MMI::KeyEvent>)> keyEventMonitor)
{
    if (keyEventMonitor == nullptr) {
        MMI_LOG_E("InputEventMonitorManager::%{public}s param should not be null!", __func__);
    ...
    MMI_LOG_D("InputEventMonitorManager::%{public}s monitorId = %{public}d", __func__, monitorId);
...
{
	if (monitorId <=0 ) {
		MMI_LOG_E("InputEventMonitorManager::%{public}s monitorId invalid", __func__);
	    return;
	}
    MonitorItem monitorItem;
    monitorItem.id_ = monitorId;
    ...
    if (iter == monitors_.end()) {
        MMI_LOG_E("InputEventMonitorManager::%{public}s monitorItem does not exist", __func__);
    } else {
        iter = monitors_.erase(iter);
        MMIEventHdl.RemoveInputEventMontior(OHOS::MMI::InputEvent::EVENT_TYPE_KEY);
        MMI_LOG_D("InputEventMonitorManager::%{public}s monitorItem id: %{public}d removed", __func__, monitorId);
    }
}
...
int32_t InputEventMonitorManager::OnMonitorInputEvent(std::shared_ptr<OHOS::MMI::KeyEvent> keyEvent)
{
    ...
    for (iter = monitors_.begin(); iter != monitors_.end(); iter++) {
        MMI_LOG_D("InputEventMonitorManager::%{public}s SendMsg", __func__);
    ...
    std::function<void (std::shared_ptr<OHOS::MMI::PointerEvent>)> TouchPadEventMonitor)
{
    ...
    MMI_LOG_D("Client AddInputEventTouchpadMontior");
...
void InputEventMonitorManager::RemoveInputEventTouchpadMontior(int32_t monitorId)
{
    ...
    if (iter == monitors_.end()) {
        ...
        MMI_LOG_D("InputEventMonitorManager::%{public}s monitorItem id: %{public}d removed", __func__, monitorId);
        MMI_LOG_D("Client RemoveInputEventTouchpadMontior");
    }
}
