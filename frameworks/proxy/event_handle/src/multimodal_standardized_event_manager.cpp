    ...
    int32_t windowId, StandEventPtr standardizedEventHandle)
{
    ...
    CHKR(typeId > 0, VAL_NOT_EXP, MMI_STANDARD_EVENT_INVALID_PARAMETER);
    ...
    MMI_LOGT("Register app event:typeId=%{public}d;", typeId);
    ...
    */
    ...
    OHOS::MMI::NetPacket ck(ID_MSG_REGISTER_MSGHANDLER);
    ck << typeId << abilityId << windowId << bundlerName << appName;
    SendMsg(ck);
    ...
    int32_t windowId, StandEventPtr standardizedEventHandle)
{
    ...
    OHOS::MMI::NetPacket ck(ID_MSG_UNREGISTER_MSGHANDLER);
    ck << typeId;
    SendMsg(ck);
    return OHOS::MMI_STANDARD_EVENT_SUCCESS;
}
...
int32_t OHOS::MMI::MultimodalStandardizedEventManager::OnKey(const KeyEvent& event)
{
    ...
    auto range = mapEvents_.equal_range(ID_MSG_KEYEVENTHANDLE_BEGIN);
...
int32_t OHOS::MMI::MultimodalStandardizedEventManager::OnTouch(const TouchEvent& event)
{
    ...
    auto range = mapEvents_.equal_range(ID_MSG_TOUCHEVENTHANDLE_BEGIN);
    for (auto i = range.first; i != range.second; ++i) {
...
int32_t OHOS::MMI::MultimodalStandardizedEventManager::OnShowMenu(const MultimodalEvent& event)
{
    ...
    auto range = mapEvents_.equal_range(ID_MSG_COMMONEVENTHANDLE_BEGIN);
    for (auto i = range.first; i != range.second; ++i) {
...
int32_t OHOS::MMI::MultimodalStandardizedEventManager::OnPlay(const MultimodalEvent& event)
{
    ...
    auto range = mapEvents_.equal_range(ID_MSG_MEDIAEVENTHANDLE_BEGIN);
    for (auto i = range.first; i != range.second; ++i) {
...
int32_t OHOS::MMI::MultimodalStandardizedEventManager::OnScreenShot(const MultimodalEvent& event)
{
    ...
    auto range = mapEvents_.equal_range(ID_MSG_SYSTEMEVENTHANDLE_BEGIN);
    for (auto i = range.first; i != range.second; ++i) {
...
int32_t OHOS::MMI::MultimodalStandardizedEventManager::OnAnswer(const MultimodalEvent& event)
{
    ...
    auto range = mapEvents_.equal_range(ID_MSG_TELEPHONEEVENTHANDLE_BEGIN);
    for (auto i = range.first; i != range.second; ++i) {
...
int32_t MultimodalStandardizedEventManager::OnDeviceAdd(const DeviceEvent& event)
{
    ...
    auto range = mapEvents_.equal_range(ID_MSG_DEVICE_BEGIN);
    for (auto i = range.first; i != range.second; ++i) {
                                                             ...
                                                             int32_t keyDownDuration, int32_t maxKeyCode)
{
    ...
    OHOS::MMI::NetPacket ckv(ID_MSG_ON_VIRTUALKEY);
    ckv << virtualevent;
    return SendMsg(ckv);
}
...
int32_t MultimodalStandardizedEventManager::InjectEvent(const KeyEvent& keyEvent)
{
    ...
    OHOS::MMI::NetPacket ckv(ID_MSG_INJECTKEYEVENT);
    ckv << virtualevent;
    return SendMsg(ckv);
}
