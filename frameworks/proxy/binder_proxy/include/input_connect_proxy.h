...
#ifndef INPUT_CONNECT_PROXY_H
#define INPUT_CONNECT_PROXY_H
...
namespace OHOS {
namespace MMI {
class InputConnectProxy final : public IRemoteProxy<IInputConnect> {
public:
    explicit InputConnectProxy(const sptr<IRemoteObject> &impl);
    DISALLOW_COPY_AND_MOVE(InputConnectProxy);
    ~InputConnectProxy() override = default;
    ...
    int32_t AddInterceptorHandler(HandleEventType eventType, int32_t priority, uint32_t deviceTags) override;
    int32_t RemoveInterceptorHandler(HandleEventType eventType, int32_t priority, uint32_t deviceTags) override;
