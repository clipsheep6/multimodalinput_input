...
void InputManagerPointerTest::TearDown()
{
    ...
    InputManager::GetInstance()->SetPointerSizeGlobal(prePointerSize_);
...
HWTEST_F(InputManagerPointerTest, InputManagerPointerTest_SetPointerSize_001, TestSize.Level1)
{
    CALL_TEST_DEBUG;
    int32_t setSize = 3;
    ASSERT_TRUE(InputManager::GetInstance()->SetPointerSizeGlobal(setSize) == RET_OK);
    setSize = 1;
    InputManager::GetInstance()->SetPointerSizeGlobal(setSize);
}
