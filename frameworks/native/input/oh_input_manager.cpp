...
static std::map<InputEvent_AxisBehavior, std::set<Input_AxisEventCallback>> g_axisMonitorCallbacks;
...
static std::shared_ptr<OHOS::MMI::OHInputInterceptor> g_interceptor = std::make_shared<OHOS::MMI::OHInputInterceptor>();
...
static bool g_hasSetRemoveMonitor { false };
    ...
    struct Input_AxisEvent *axisEvent, int32_t axisEventType)
{
    ...
    if (axisEventType == OHOS::MMI::PointerEvent::AXIS_EVENT_TYPE_PINCH) {
        ...
        MMI_HILOGE("Undefined behavior: %{public}d", behavior);
        return false;
    }
                                       ...
                                       InputEvent_AxisEventType* axisEventType)
{
    ...
    *axisEventType = axisEvent->axisEventType;
    return INPUT_SUCCESS;
}
                                             ...
                                             InputEvent_SourceType* sourceType)
{
    ...
    *sourceType = axisEvent->sourceType;
    return INPUT_SUCCESS;
}
...
static void AxisEventMonitorCallback(std::shared_ptr<OHOS::MMI::PointerEvent> event)
{
    ...
    auto it = g_axisMonitorCallbacks.find(InputEvent_AxisBehavior(event->GetAxisBeHavior()));
...
static int32_t AddPointerEventMonitor()
{
    if (g_pointerMonitorId == INVALID_MONITOR_ID) {
...
Input_Result OH_Input_AddMouseEventMonitor(Input_MouseEventCallback callback)
{
    ...
    CHKPR(callback, INPUT_PARAMETER_ERROR);
    std::lock_guard guard(g_mutex);
    int32_t ret = AddPointerEventMonitor();
...
static bool IsAllCallbacksEmpty()
{
...
static void RemovePointerEventMonitor()
{
    if (IsAllCallbacksEmpty()) {
        OHOS::Singleton<OHOS::MMI::InputManagerImpl>::GetInstance().RemoveMonitor(g_pointerMonitorId);
        g_pointerMonitorId = INVALID_MONITOR_ID;
    }
}
...
Input_Result OH_Input_RemoveAxisEventMonitor(InputEvent_AxisEventType axisEventType, Input_AxisEventCallback callback)
{
    ...
    CHKPR(callback, INPUT_PARAMETER_ERROR);
    std::lock_guard guard(g_mutex);
    if (g_axisMonitorCallbacks.find(axisBehavior) == g_axisMonitorCallbacks.end()) {
        return INPUT_PARAMETER_ERROR;
    }
    auto it = g_axisMonitorCallbacks[axisBehavior].find(callback);
    if (it == g_axisMonitorCallbacks[axisBehavior].end()) {
        return INPUT_PARAMETER_ERROR;
    }
    g_axisMonitorCallbacks[axisBehavior].erase(it);
    if (g_axisMonitorCallbacks[axisBehavior].empty()) {
        g_axisMonitorCallbacks.erase(axisBehavior);
    }
    RemovePointerEventMonitor();
    return INPUT_SUCCESS;
}
...
Input_Result OH_Input_AddKeyEventInterceptor(Input_KeyEventCallback callback, struct Input_InterceptorOptions *option)
{
    ...
    std::lock_guard guard(g_mutex);
    int32_t ret = OHOS::Singleton<OHOS::MMI::InputManagerImpl>::GetInstance().AddInterceptor(KeyEventInterceptorCallback);
    if (ret < RET_OK) {
        return INPUT_SERVICE_EXCEPTION;
    }
...
int32_t OH_Input_AddInputEventInterceptor(struct Input_InterceptorEventCallback *callback)
{
    CALL_DEBUG_ENTER;
    CHKPR(callback, INPUT_PARAMETER_ERROR);
    ...
    std::lock_guard guard(g_mutex);
    g_interceptor->SetCallback(PointerEventInterceptorCallback);
    int32_t ret = g_interceptor->Start();
    if (ret < RET_OK) {
        return INPUT_SERVICE_EXCEPTION;
    }
...
int32_t OH_Input_RemoveInputEventInterceptor(struct Input_InterceptorEventCallback *callback)
{
    CALL_DEBUG_ENTER;
    CHKPR(callback, INPUT_PARAMETER_ERROR);
    std::lock_guard guard(g_mutex);
