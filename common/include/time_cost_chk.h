    ...
    ~TimeCostChk(void)
    {
        ...
        if ((ullCost > uiTime_) && strReason_.size() > 0 && strOutput_.size() > 0) {
            if (0 != llParam1_ || 0 != llParam2_) {
                MMI_LOGW("Time cost overtime (%{public}lu(us)>%{public}u(us)) when Reason:%{public}s chk:%{public}s "
                         "param1:%{public}u param2:%{public}u",
                         ullCost, uiTime_, strReason_.c_str(), strOutput_.c_str(), llParam1_, llParam2_);
            } else {
                MMI_LOGW("TimeCostChk Overtime(%{public}lu(us)>%{public}u(us)) when Reason:%{public}s chk:%{public}s",
