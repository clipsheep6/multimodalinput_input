...
int32_t HdfDeviceEventDispatch::EventPkgCallback(const std::vector<Eventpackage> &pkgs, unit32_t devIndex)
{
    if (pkgs.empty()) {
...
int32_t HdfDeviceEventDispatch::HotPlugCallback(const HotPlugCallback &event)
{
    return HDF_SUCCESS;
}
} // namespace MMI
