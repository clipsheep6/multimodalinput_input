...
bool InputEventNormalizeHandler::CheckKeyboardWhiteList(const std::shared_ptr<KeyEvent> keyEvent) {
    CALL_DEBUG_ENTER;
#ifdef OHOS_BUILD_ENABLE_COOPERATE
    int32_t deviceId = keyEvent->GetDeviceId();
    auto su = InputDevMgr->GetDeviceSupportKey(deviceId);
    MMI_HILOGI("GetDeviceSupportKey: su : %{public}d", su);
    if (KEYBOARD_TYPE_ALPHABETICKEYBOARD != su) {
       return true;
    }
