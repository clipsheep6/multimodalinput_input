...
void EventNormalizeHandler::HandleEvent(libinput_event* event)
{
    ...
    if ((type == LIBINPUT_EVENT_POINTER_TAP) &&
        (MulFingersTapHdr->GetMultiFingersState() == MulFingersTap::TRIPLETAP)) {
        MulFingersTapHdr->SetMulFingersTapHdrDefault();
        return;
    }
...
int32_t EventNormalizeHandler::HandleTouchPadEvent(libinput_event* event)
{
    ...
    MulFingersTapHdr->HandleMulFingersTap(touchpad, type);
    auto pointerEvent = TouchEventHdr->OnLibInput(event, TouchEventNormalize::DeviceType::TOUCH_PAD);
    CHKPR(pointerEvent, ERROR_NULL_POINTER);
    if (MulFingersTapHdr->GetMultiFingersState() == MulFingersTap::TRIPLETAP) {
        nextHandler_->HandlePointerEvent(pointerEvent);
        MulFingersTapHdr->ClearPointerItems(pointerEvent);
    }
    ...
    if (buttonIds_.empty()) {
        MulFingersTapHdr->SetMulFingersTapHdrDefault(false);
    }
    return RET_OK;
#else
    MMI_HILOGW("Pointer device does not support");
