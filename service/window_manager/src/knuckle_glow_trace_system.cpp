...
KnuckleGlowTraceSystem::KnuckleGlowTraceSystem(int pointsize, const OHOS::Rosen::Drawing::Bitmap &shadow, int maxDivergenceNum)
{
    CALL_DEBUG_ENTER;
    for (int i = 0; i < pointsize; ++i) {
        mDivergentPoints_.emplace_back(std::make_shared<KnukleDivergentPoint>(shadow));
        mGlowPoints_.emplace_back(std::make_shared<KnukleGlowPoint>(shadow));
    }
...
void KnuckleGlowTraceSystem::SetTrackColor(int trackColor)
{
    CALL_DEBUG_ENTER;
    for (auto particle : mGlowPoints_) {
        if (particle != nullptr) {
...
void KnuckleGlowTraceSystem::AddGlowPoints(Rosen::Drawing::Path path, long timeInteval)
{
    ...
    for (auto glowPoint : mGlowPoints_) {
        if (glowPoint == nullptr) {
            MMI_HILOGE("glowPoint null");
        }
        if (!glowPoint->IsEnded()) {
            MMI_HILOGE("glowPoint not IsEnded");
        }
        MMI_HILOGE("AddGlowPoints start pathlength:%{public}f  distanceFromEnd:%{public}f", pathlength, distanceFromEnd);
        if (glowPoint != nullptr && glowPoint->IsEnded() && distanceFromEnd <= pathlength) {
            if (path.GetPositionAndTangent(distanceFromEnd, pathPoints, tangent, true)) {
                MMI_HILOGE("glowPoint->Reset GetX:%{public}f  GetY:%{public}f, lifespanOffset:%{public}f", pathPoints.GetX(), pathPoints.GetY(), lifespanOffset);
