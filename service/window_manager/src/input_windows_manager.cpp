...
void InputWindowsManager::AdjustDisplayRotation()
{
    if (!TOUCH_DRAWING_MGR->IsWindowRotation()) {
        ...
        if (cursorPos.direction != displayInfo->direction) {
            ...
            cursorPos.direction = displayInfo->direction;
            UpdateAndAdjustMouseLocation(cursorPos.displayId, coord.x, coord.y);
            IPointerDrawingManager::GetInstance()->UpdateDisplayInfo(*displayInfo);
            IPointerDrawingManager::GetInstance()->SetPointerLocation(
                static_cast<int32>(coord.x), static_cast<int32>(coord.y));
        }
    }
}
...
void InputWindowsManager::RotateScreen(const DisplayInfo& info, PhysicalCoordinate& coord) const
{
    ...
    if (direction == DIRECTION0) {
        if (!TOUCH_DRAWING_MGR->IsWindowRotation() && cursorPos.direction != displayInfo->direction) {
            if (cursorPos.direction == Direction::DIRECTION90) {
                double temp = coord.y;
                coord.y = info.height - coord.x;
                coord.x = temp;
            } else if (cursorPos.direction == Direction::DIRECTION270) {
                double temp = coord.x;
...
int32_t InputWindowsManager::UpdateMouseTarget(std::shared_ptr<PointerEvent> pointerEvent)
{
    ...
    cursorPos.direction = physicalDisplayInfo->direction;
    int64_t endTime = GetSysClockTime();
    if ((endTime - beginTime) > RS_PROCESS_TIMEOUT) {
        MMI_HILOGW("Rs process timeout");
    }
