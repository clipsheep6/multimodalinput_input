...
void TouchDrawingManager::TouchDrawHandler(const std::shared_ptr<PointerEvent>& pointerEvent)
{
    ...
    CreateObserver();
    if (bubbleMode_.isShow && bubbleCanvasNode_ == nullptr) {
        bubbleCanvasNode_ = Rosen::RSCanvasNode::Create();
        CreateCanvasNode(bubbleCanvasNode_);
    }
    if (pointerMode_.isShow && pointerCanvasNode_ == nullptr) {
        pointerCanvasNode_ = Rosen::RSCanvasDrawingNode::Create();
        CreateCanvasNode(pointerCanvasNode_);
    }    
    CreateTouchWindow();
...
void TouchDrawingManager::DrawBubble()
{
    ...
    auto canvas = CreateCanvas(bubbleCanvasNode_);
    CHKPV(canvas);
...
void TouchDrawingManager::ClearTracker()
{
    ...
    if (lastPointerItem_.empty() && isDownAction_) {
       auto canvas = static_cast<Rosen::RSCanvasDrawingNode*>(pointerCanvasNode_.get());
       canvas->ResetSurface();
    }
}
...
void TouchDrawingManager::UpdateLabels(bool isShow)
{
    if (isShow) {
        CreateCanvasNode(pointerCanvasNode_);
        CreateTouchWindow();
        trackerCanvas_ = CreateCanvas(pointerCanvasNode_);
        DrawLabels();
    } else {
...
void TouchDrawingManager::CreateCanvasNode(std::shared_ptr<Rosen::RSCanvasNode>& canvasNode)
{
...
#endif
TouchDrawingManager::CreateCanvas(const std::shared_ptr<Rosen::RSCanvasNode>& canvasNode)
{
#ifndef USE_ROSEN_DRAWING
    return static_cast<Rosen::RSRecordingCanvas *>(canvasNode->
        BeginRecording(displayInfo_.width, displayInfo_.height));
#else
    return static_cast<Rosen::Drawing::RecordingCanvas *>(canvasNode->
        BeginRecording(displayInfo_.width, displayInfo_.height));
#endif
}
