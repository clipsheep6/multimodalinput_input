...
{
	MMI_HILOGD("InputDisplayBindHelper:InputDisplayBindHelper");
}

std::string InputDisplayBindHelper::GetBindDisplayNameByInputDevice(int32_t inputDeviceId) const 
{
	CALL_DEBUG_ENTER;
	MMI_HILOGD("Param: inputDeviceId:%{public}d", inputDeviceId);
	if (infos_ == nullptr) {
		return {};
	}
...
void InputDisplayBindHelper::Store()
{
	...
	MMI_HILOGEK("Open file for write begin:%{public}s", fileName_.c_str());
	...
	MMI_HILOGEK("Open file for write end:%{public}s", fileName_.c_str());
	ofs << *infos_;
	ofs.close();
}
...
int32_t InputDisplayBindHelper::SetDisplayBind(int32_t deviceId, int32_t displayId, std::string &msg)
{
	...
	// 前面全部解绑时, 可能输入设备与显示屏都绑定有对应的设备, 所以, 此处直接绑定他们剩下的
...
{
	CALL_DEBUG_ENTER;
	MMI_HILOGEK("Open file begin:%{public}s", fileName_.c_str());
