...
bool InputDisplayBindHelperTest::InitInputNode()
{
    ...
    if (!file.is_open()) {
        MMI_HILOGE("write fail");
        return false;
    }
    file << INPUT_NODE_NAME;
    file.close();
    MMI_HILOGI("write success");
    return true;
}
...
HWTEST_F(InputDisplayBindHelperTest, InputDisplayBindHelperTest_GetInputDeviceById_007, TestSize.Level1)
{
    ...
    // 读取输入节点名称
    std::string str1 = idh.GetContent(INPUT_DEVICE_NAME_FILE);
    ASSERT_EQ(str1, INPUT_NODE_NAME);
    // 根据输入节点名称获取输入节点
    std::string str2 = idh.GetInputNode(INPUT_NODE_NAME);
    ASSERT_EQ(str2, "");
    // 根据id获取输入节点名称
    std::string str3 = idh.GetInputNodeNameByCfg(1000);
    ASSERT_EQ(str3, "");
    // 根据id获取输入设备
    std::string str4 = idh.GetInputDeviceById(1000);
    ASSERT_EQ(str4, "");
}
} // namespace MMI
} // namespace OHOS