...
int32_t HdiInject::OnSetEventInject(const RawInputEvent& allEvent, int32_t devIndex)
{
    ...
    HdiInjectObj->eventcallback_.EventPkgCallback((const EventPackage**)pack, 1, devIndex);
    free(pack[0]);
...
bool HdiInject::SyncDeviceHotStatus()
{
    ...
    for (int32_t i = 0; i < count; i++) {
        ...
        HdiInjectObj->hotPlugcallback_.HotPlugCallback((const HotPlugEvent*)&event_[i]);
    }
...
{
    SyncDeviceHotStatus();
    HdiInjectObj->hotPlugcallback_.HotPlugCallback(*event_);
...
bool HdiInject::ReportHotPlugEvent(uint32_t devIndex, uint32_t status)
{
    ...
    HdiInjectObj->hotPlugcallback_.HotPlugCallback(&event);
...
{
    return HdiInjectObj->ScanInputDevice(staArr, arrLen);
}
...
{
    HdiInjectObj->eventcallback_.EventPkgCallback = callback->EventPkgCallback;
...
{
    HdiInjectObj->hotPlugcallback_.HotPlugCallback = callback->HotPlugCallback;
