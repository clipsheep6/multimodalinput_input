...
namespace MMI {
namespace {
    static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, MMI_LOG_DOMAIN, "PrepareDInputCallback" };
}

PrepareDInputCallback::PrepareDInputCallback(const int32_t& taskId, SessionPtr &sess)
{
    taskId_ = taskId;
    sess_ = sess;
}
...
void PrepareDInputCallback::OnResult(const std::string& deviceId, const int32_t& status)
{
    ...
    if (!sess_->SendMsg(pkt2)) {
        MMI_LOGE("PrepareDInputCallback::OnResult Sending failed!\n");
    }
    MMI_LOGD("PrepareDInputCallback::OnResult end");
}
}  // namespace MMI
}  // namespace OHOS
