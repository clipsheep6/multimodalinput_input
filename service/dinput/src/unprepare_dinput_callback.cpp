...
namespace MMI {
namespace {
    static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, MMI_LOG_DOMAIN, "UnprepareDInputCallback" };
}

UnprepareDInputCallback::UnprepareDInputCallback(const int32_t& taskId, SessionPtr& sess)
{
    taskId_ = taskId;
    sess_ = sess;
}
...
void UnprepareDInputCallback::OnResult(const std::string& deviceId,const int32_t& status)
{
    ...
    if (!sess_->SendMsg(pkt2)) {
        MMI_LOGE("UnprepareDInputCallback::OnResult Sending failed!\n");
    }
    MMI_LOGD("UnprepareDInputCallback::OnResult end");
}
}  // namespace MMI
}  // namespace OHOS
