...
int32_t MultimodalInputConnectStub::StubSetNapStatus(MessageParcel& data, MessageParcel& reply)
{
    ...
    MMI_HILOGD("Success napStatus:%{public}d, pid:%{public}d", napStatus, GetCallingPid());
    return RET_OK;
}
...
int32_t MultimodalInputConnectStub::StubGetAllNapStatusData(MessageParcel& data, MessageParcel& reply)
{
    ...
    for (const auto &data : datas) {
        WRITEINT32(reply, data.pid, ERR_INVALID_VALUE);
        WRITEINT32(reply, data.uid, ERR_INVALID_VALUE);
        WRITESTRING(reply, data.bundleName, ERR_INVALID_VALUE);
    }
    return RET_OK;
}
