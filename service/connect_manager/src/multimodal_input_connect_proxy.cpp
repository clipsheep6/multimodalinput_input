...
int32_t MultimodalInputConnectProxy::GetAllSystemShortcutKey(std::vector<std::unique_ptr<KeyOption>> &keyOptions)
{
    CALL_DEBUG_ENTER;
    MessageParcel data;
    if (!data.WriteInterfaceToken(MultimodalInputConnectProxy::GetDescriptor())) {
    ...
    sptr<IRemoteObject> remote = Remote();
    CHKPR(remote, RET_ERR);
    int32_t ret = remote->SendRequest(static_cast<uint32_t>(MultimodalinputConnectInterfaceCode::GET_SYSTEM_SHORTCUT_KEY),
        data, reply, option);
    if (ret != RET_OK) {
        MMI_HILOGE("Send request failed, ret:%{public}d", ret);
        return ret;
    ...
    MMI_HILOGE("keyOptionsCount size:%{public}d", keyOptionsCount);
    return RET_OK;
}
} // namespace MMI
} // namespace OHOS
