...
int32_t MultimodalInputConnectProxy::RegisterCooperateEvent(sptr<IEventCooperate> event)
{
    CALL_DEBUG_ENTER;
    MessageParcel data;
    if (!data.WriteInterfaceToken(MultimodalInputConnectProxy::GetDescriptor())) {
    ...
    if (!data.WriteRemoteObject(event->AsObject().GetRefPtr())) {
        MMI_HILOGE("Failed to write event");
        return ERR_INVALID_VALUE;
    }
    MessageParcel reply;
    MessageOption option;
...
int32_t MultimodalInputConnectProxy::StartInputDeviceCooperate(const std::string &sinkDeviceId, int32_t srcInputDeviceId)
{
    CALL_DEBUG_ENTER;
    MessageParcel data;
    if (!data.WriteInterfaceToken(MultimodalInputConnectProxy::GetDescriptor())) {
