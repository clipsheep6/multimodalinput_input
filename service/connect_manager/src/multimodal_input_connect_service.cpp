...
int32_t MultimodalInputConnectService::AllocSocketFd(const std::string &programName, const int moduleType, int &toReturnClientFd)
{
    MMI_LOGI("MultimodalInputConnectService::AllocSocketFd enter, programName: %{public}s, moduleType: %{public}d", programName.c_str(), moduleType);
    ...
    MMI_LOGI("leave, programName: %{public}s, moduleType: %{public}d, alloc success.", programName.c_str(), moduleType);
...
void MultimodalInputConnectService::OnStart()
{
    ...
    MMI_LOGI("Congratulations, MultimodalInputConnectService start successfully!");
}
...
int32_t MultimodalInputConnectService::HandleAllocSocketFd(MessageParcel& data, MessageParcel& reply)
{
    ...
    MMI_LOGE("clientName = %{public}s, moduleId = %{public}d", req->data.clientName.c_str(), req->data.moduleId);
