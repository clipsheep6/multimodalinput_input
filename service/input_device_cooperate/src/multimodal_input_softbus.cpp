...
namespace MMI {
namespace {
std::shared_ptr<MultimodalInputSoftbus> g_instance = nullptr;
constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {LOG_CORE, MMI_LOG_DOMAIN, "MultimodalInputSoftbus"};
...
{
    return SoftbusMgr->OnSessionOpened(sessionId, result);
}

static void SessionClosed(int32_t sessionId)
{
    SoftbusMgr->OnSessionClosed(sessionId);
}

static void BytesReceived(int32_t sessionId, const void *data, uint32_t dataLen)
{
    SoftbusMgr->OnBytesReceived(sessionId, data, dataLen);
}
...
int32_t MultimodalInputSoftbus::Init()
{
    CALL_INFO_TRACE;
    sessListener_ = {
    ...
    int32_t ret = CreateSessionServer(MMI_PKG_NAME, mySessionName_.c_str(), &sessListener_);
    if (ret != RET_OK) {
...
void MultimodalInputSoftbus::Release()
{
    std::unique_lock<std::mutex> sessionLock(operationMutex_);
    std::for_each(sessionDevMap_.begin(), sessionDevMap_.end(), [](auto item) { CloseSession(item.second); });
    (void)RemoveSessionServer(MMI_PKG_NAME, mySessionName_.c_str());
    sessionDevMap_.clear();
    channelStatusMap_.clear();
}

int32_t MultimodalInputSoftbus::CheckDeviceSessionState(const std::string &devId)
{
    std::unique_lock<std::mutex> sessionLock(operationMutex_);
...
int32_t MultimodalInputSoftbus::OpenInputSoftbus(const std::string &remoteDevId)
{
    CALL_INFO_TRACE;
...
void MultimodalInputSoftbus::CloseInputSoftbus(const std::string &remoteDevId)
{
    std::unique_lock<std::mutex> sessionLock(operationMutex_);
...
std::shared_ptr<MultimodalInputSoftbus> MultimodalInputSoftbus::GetInstance()
{
    static std::once_flag flag;
    std::call_once(flag, [&]() {
        g_instance.reset(new (std::nothrow) MultimodalInputSoftbus());
    });
    return g_instance;
}


int32_t MultimodalInputSoftbus::StartRemoteCooperate(const std::string &localDeviceId,
...
int32_t MultimodalInputSoftbus::StartRemoteCooperateResult(const std::string &remoteDeviceId, bool isSuccess,
...
int32_t MultimodalInputSoftbus::StopRemoteCooperate(const std::string &remoteDeviceId)
{
    CALL_DEBUG_ENTER;
    std::unique_lock<std::mutex> sessionLock(operationMutex_);
    if (sessionDevMap_.count(remoteDeviceId) > 0) {
...
int32_t MultimodalInputSoftbus::StopRemoteCooperateResult(const std::string &remoteDeviceId, bool isSuccess)
{
    CALL_DEBUG_ENTER;
    std::unique_lock<std::mutex> sessionLock(operationMutex_);
    if (sessionDevMap_.count(remoteDeviceId) > 0) {
...
int32_t MultimodalInputSoftbus::StartCooperateOtherResult(const std::string &remoteDeviceId,
...
void MultimodalInputSoftbus::HandleSessionData(int32_t sessionId, const std::string& message)
{
...
void MultimodalInputSoftbus::OnBytesReceived(int32_t sessionId, const void *data, uint32_t dataLen)
{
    ...
    MMI_HILOGD("OnBytesReceived message:%{public}ss.", message.c_str());
...
int32_t MultimodalInputSoftbus::SendMsg(int32_t sessionId, std::string &message)
{
    CALL_DEBUG_ENTER;
    if (message.size() > MSG_MAX_SIZE) {
        MMI_HILOGW("SendMessage error: message.size() > MSG_MAX_SIZE");
        return RET_ERR;
    }
    uint8_t *buf = (uint8_t *)calloc((MSG_MAX_SIZE), sizeof(uint8_t));
    if (buf == nullptr) {
        MMI_HILOGE("SendMsg: malloc memory failed");
        return RET_ERR;
    }
    int32_t outLen = 0;
    if (memcpy_s(buf, MSG_MAX_SIZE, (const uint8_t *)message.c_str(), message.size()) != RET_OK) {
        MMI_HILOGE("SendMsg: memcpy memory failed");
        free(buf);
        return RET_ERR;
    }
...
std::string MultimodalInputSoftbus::FindDeviceBySession(int32_t sessionId)
{
    std::unique_lock<std::mutex> sessionLock(operationMutex_);
...
int32_t MultimodalInputSoftbus::OnSessionOpened(int32_t sessionId, int32_t result)
{
    CALL_INFO_TRACE;
...
void MultimodalInputSoftbus::OnSessionClosed(int32_t sessionId)
{
    CALL_DEBUG_ENTER;
    std::string deviceId = FindDeviceBySession(sessionId);
    std::unique_lock<std::mutex> sessionLock(operationMutex_);
...
void MultimodalInputSoftbus::NotifyResponseStartRemoteCooperate(int32_t sessionId, const nlohmann::json &recMsg)
{
    CALL_DEBUG_ENTER;
...
void MultimodalInputSoftbus::NotifyResponseStartRemoteCooperateResult(int32_t sessionId, const nlohmann::json &recMsg)
{
    CALL_DEBUG_ENTER;
...
void MultimodalInputSoftbus::NotifyResponseStopRemoteCooperate(int32_t sessionId, const nlohmann::json &recMsg)
{
    InputDevCooSM->StopRemoteCooperate();
}

void MultimodalInputSoftbus::NotifyResponseStopRemoteCooperateResult(int32_t sessionId, const nlohmann::json &recMsg)
{
    CALL_DEBUG_ENTER;
    if (!recMsg[MMI_SOFTBUS_KEY_RESULT].is_boolean()) {
        MMI_HILOGE("OnBytesReceived cmdType is TRANS_SINK_MSG_ONPREPARE, data type is error.");
...
void MultimodalInputSoftbus::NotifyResponseStartCooperateOtherResult(int32_t sessionId, const nlohmann::json &recMsg)
{
    CALL_DEBUG_ENTER;
