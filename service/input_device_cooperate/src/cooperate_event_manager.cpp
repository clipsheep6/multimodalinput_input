...
void CooperateEventManager::AddCooperationEvent(sptr<EventInfo> event)
{
    ...
    if (event->type == EventType::LISTENER) {
        cooperateListenerList_.emplace_back(event);
    } else {
        cooperateCallbackList_[event->type] = event;
    }
}
...
void CooperateEventManager::RemoveCooperationListenerEvent(sptr<EventInfo> event)
{
    CALL_DEBUG_ENTER;
    std::lock_guard<std::mutex> guard(lock_);
    if (cooperateListenerList_.size() < 0 || !event) {
...
void CooperateEventManager::OnEnable(CooperationMessage msg, const std::string &deviceId)
{
    CALL_DEBUG_ENTER;
    std::lock_guard<std::mutex> guard(lock_);
    sptr<EventInfo> info = cooperateCallbackList_[EventType::ENABLE];
    CHKPV(info);
    NotifyCooperateMessage(info->sess, info->msgId, info->userData, deviceId, msg);
    cooperateCallbackList_[EventType::ENABLE] =  nullptr;
}
...
void CooperateEventManager::OnStart(CooperationMessage msg, const std::string &deviceId)
{
    CALL_DEBUG_ENTER;
    std::lock_guard<std::mutex> guard(lock_);
    sptr<EventInfo> info = cooperateCallbackList_[EventType::START];
    CHKPV(info);
    NotifyCooperateMessage(info->sess, info->msgId, info->userData, deviceId, msg);
    cooperateCallbackList_[EventType::START] =  nullptr;
}
...
void CooperateEventManager::OnStop(CooperationMessage msg, const std::string &deviceId)
{
    CALL_DEBUG_ENTER;
    std::lock_guard<std::mutex> guard(lock_);
    sptr<EventInfo> info = cooperateCallbackList_[EventType::STOP];
    CHKPV(info);
    NotifyCooperateMessage(info->sess, info->msgId, info->userData, deviceId, msg);
    cooperateCallbackList_[EventType::STOP] =  nullptr;
}
