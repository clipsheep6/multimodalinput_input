...
{
    simulationEventListener_ = new (std::nothrow) SimulationEventCallbackImpl();
...
int32_t DistributedInputAdapter::RegisterEventCallback(SimulationEventCallback callback)
{
    std::lock_guard<std::mutex> guard(adapterLock_);
    CHKPR(callback, RET_ERR);
    simulationEventCallback_ = callback;
    return RET_OK;
}
int32_t DistributedInputAdapter::UnregisterEventCallback(SimulationEventCallback callback)
{
    std::lock_guard<std::mutex> guard(adapterLock_);
    CHKPR(callback, RET_ERR);
...
{
    std::lock_guard<std::mutex> guard(adapterLock_);
    CHKPV(simulationEventCallback_);
    simulationEventCallback_(type, code, value);
}
...
int32_t DistributedInputAdapter::SimulationEventCallbackImpl::OnSimulationEvent(uint32_t type, uint32_t code,
    int32_t value)
{
