...
void FingersenseWrapper::InitFingerSenseWrapper()
{
    ...
    char *error = nullptr;
    if ((error = dlerror()) != nullptr || fingerSenseWrapperHandle_ == nullptr) {
        MMI_HILOGE("libfingersense_wrapper.z.so was not loaded, error: %{public}s", error);
        return;
    }

    setCurrentToolType_ = (SET_CURRENT_TOOL_TYPE)dlsym(fingerSenseWrapperHandle_, "SetCurrentToolType");
    knuckleTouchUp_ = (KNUCKLE_TOUCH_UP)dlsym(fingerSenseWrapperHandle_, "knuckleTouchUp");
    ...
    if ((error = dlerror()) != nullptr || setCurrentToolType_ == nullptr || knuckleTouchUp_ == nullptr ||
        enableFingersense_ == nullptr || disableFingerSense_) {
        MMI_HILOGE("fingersense wrapper symbol failed, error: %{public}s", error);
        return;
    }
