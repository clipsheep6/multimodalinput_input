...
TouchScreenEventHandler::TouchScreenEventHandler(std::shared_ptr<IInputContext> context) : context_(context)
{
    mtdev_ = input_mtdev_open();
}

std::shared_ptr<TouchScreenEventHandler> TouchScreenEventHandler::CreateInstance(std::shared_ptr<IInputContext> context)
{
    return std::shared_ptr<TouchScreenEventHandler>(new TouchScreenEventHandler(context));
}
...
bool TouchScreenEventHandler::OnEventTouchDown(std::shared_ptr<Slot> slot)
{
    CHKPF(slot);
    CHKPF(context_);
    auto winMgr = context_->GetInputWindowsManager();
    CHKPF(winMgr);
