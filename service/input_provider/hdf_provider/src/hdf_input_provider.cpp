...
HDFInputProvider::HDFInputProvider(int32_t queueId) : IInputProvider(queueId)
{
    CHK_PID_AND_TID();
...
void HDFInputProvider::EventDispatch(epoll_event &ev)
{
    ...
    for (int32_t i = 0; i < onceProcessLimit; ++i) {
        ...
        if (event->type == 1) {
            ...
            if (ev.value == 1) {
                auto dev = std::make_shared<HDFInputDevice>(g_InputContext, ev.code, g_InputInterface, eventcb_);
                CHKPC(dev);
