...
public:
    IInputProvider(int32_t queueId) : queueId_(queueId) {}
    ...
    int32_t GetQueueId() const { return queueId_; }
protected:
    const int32_t queueId_;
...
struct IInputProviderStruct {
    std::shared_ptr<OHOS::MMI::IInputProvider> provider;
};
typedef bool GetInputProvider(int32_t queueId, IInputProviderStruct **s);
