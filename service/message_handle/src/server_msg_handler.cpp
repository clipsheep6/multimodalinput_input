...
int32_t OHOS::MMI::ServerMsgHandler::OnSeniorInputFuncProc(SessionPtr SessionPtr, NetPacket& pkt)
{
	...
	if (processResult) {
		...
		MMI_LOGE("process fail, fd: %{public}d, msgType: %{public}d, processResult: %{public}d.", fd, msgType, processResult);
	}

	const uint32_t responseCode = seniorInput_->ReplyMessage(SessionPtr, processResult);
	if (responseCode == RET_ERR) {
		MMI_LOGW("reply msg to client fail, fd: %{public}d, msgType: %{public}d, processResult: %{public}d, replyCode: %{public}d.",
