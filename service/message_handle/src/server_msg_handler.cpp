...
#endif
    MsgCallback funs[] = {
        {ID_MSG_REGISTER_APP_INFO, MsgCallbackBind2(&ServerMsgHandler::OnRegisterAppInfo, this)},
        {ID_MSG_REGISTER_MSGHANDLER, MsgCallbackBind2(&ServerMsgHandler::OnRegisterMsgHandler, this)},
        {ID_MSG_UNREGISTER_MSGHANDLER, MsgCallbackBind2(&ServerMsgHandler::OnUnregisterMsgHandler, this)},
        {ID_MSG_ON_WINDOWS, MsgCallbackBind2(&ServerMsgHandler::OnWindows, this)},
        {ID_MSG_ON_VIRTUALKEY, MsgCallbackBind2(&ServerMsgHandler::OnVirtualKeyEvent, this)},
        {ID_MSG_CHECKREPLYMESSAGE, MsgCallbackBind2(&ServerMsgHandler::CheckReplyMessageFormClient, this)},
        {ID_MSG_ON_DUMP, MsgCallbackBind2(&ServerMsgHandler::OnDump, this)},
        {ID_MSG_ON_LIST, MsgCallbackBind2(&ServerMsgHandler::OnListInject, this)},
        {ID_MSG_GETMMIINFO, MsgCallbackBind2(&ServerMsgHandler::GetMultimodeInputInfo, this)},
        {ID_MSG_INJECTKEYEVENT, MsgCallbackBind2(&ServerMsgHandler::OnInjectKeyEvent, this) },
#ifdef OHOS_BUILD_AI
        ...
        {ID_MSG_AUTOTEST_FRAME_REPLYPKT, MsgCallbackBind2(&ServerMsgHandler::AutoTestReceiveClientPkt, this)},
...
void OHOS::MMI::ServerMsgHandler::OnMsgHandler(SessionPtr sess, NetPacket& pkt)
{
    ...
    TimeCostChkfp1("ServerMsgHandler::OnMsgHandler", "overtime 300(us)", MAX_OVER_TIME, id);
    auto fun = GetFun(id);
    if (!fun) {
...
int32_t OHOS::MMI::ServerMsgHandler::OnSeniorInputFuncProc(SessionPtr SessionPtr, NetPacket& pkt)
{
    ...
    if (msgType == MSG_TYPE_DEVICE_INIT) {
        ...
        if (devType == INPUT_DEVICE_CAP_AISENSOR) {
            auto ptr = SeniorInputFuncProcBase::Create<AIFuncProc>();
            bool retcode = seniorInput_->DeviceInit(fd, ptr);
            if (retcode) {
                MMI_LOGI("device init success");
            } else {
                MMI_LOGE("device init fail: %{public}d", fd);
            }
        } else if (devType == INPUT_DEVICE_CAP_KNUCKLE) {
            auto ptr = SeniorInputFuncProcBase::Create<KnuckleFuncProc>();
            bool ret = seniorInput_->DeviceInit(fd, ptr);
            if (ret) {
                MMI_LOGI("device init success");
            } else {
                MMI_LOGE("device init fail: %{public}d", fd);
        ...
        bool processResult = seniorInput_->DeviceEventDispatch(fd, seniorInputEvent);
        if (processResult) {
            MMI_LOGI("device event dispatch success");
        } else {
            MMI_LOGE("device event dispatch fail: %{public}d", fd);
        }

        int32_t replyCode = seniorInput_->ReplyMessage(SessionPtr, processResult);
        if (replyCode == RET_ERR) {
            MMI_LOGE("reply msg to knuckle client fail");
            return RET_ERR;
        }
    }
    return RET_OK;
...
int32_t OHOS::MMI::ServerMsgHandler::OnHdiInject(SessionPtr sess, NetPacket& pkt)
{
    ...
    CHKR(udsServer_, NULL_POINTER, RET_ERR);
    return hdiInject->ManageHdfInject(sess, pkt);
}
#endif
...
int32_t OHOS::MMI::ServerMsgHandler::OnRegisterAppInfo(SessionPtr sess, NetPacket& pkt)
{
    ...
    NetPacket pktAutoTest(ID_MSG_AUTOTEST_FRAME_CLISTPKT);
...
int32_t OHOS::MMI::ServerMsgHandler::OnUnregisterMsgHandler(SessionPtr sess, NetPacket& pkt)
{
    ...
    int32_t fd = sess->GetFd();
    pkt >> handle;
    RegEventHM->UnregisterEventHandleManager(handle, fd);
    return RET_OK;
}

int32_t OHOS::MMI::ServerMsgHandler::OnWindows(SessionPtr sess, NetPacket& pkt)
{
    CHKR(udsServer_, NULL_POINTER, RET_ERR);
...
int32_t OHOS::MMI::ServerMsgHandler::GetMultimodeInputInfo(SessionPtr sess, NetPacket& pkt)
{
    ...
    if (tagPackHead.idMsg) {
        TagPackHead tagPackHeadAck = {0, {fd}};
        NetPacket pktAck(ID_MSG_GETMMIINFO_ACK);
...
int32_t OHOS::MMI::ServerMsgHandler::AutoTestReceiveClientPkt(SessionPtr sess, NetPacket& pkt)
{
    ...
    NetPacket pktAutoTest(ID_MSG_AUTOTEST_FRAME_CLTPKT);
