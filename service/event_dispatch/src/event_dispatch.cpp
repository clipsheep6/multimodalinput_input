...
int32_t OHOS::MMI::EventDispatch::GestureRegisteredEventDispatch(const ID_MSG& idMsg, OHOS::MMI::UDSServer& udsServer,
    RegisteredEvent& registeredEvent, uint64_t preHandlerTime)
{
    auto ret = RET_OK;
    if (idMsg == ID_MSG_ON_PREVIOUS) {
        ret = RegisteredEventDispatch(ID_MSG_ON_PREVIOUS, udsServer, registeredEvent,
            INPUT_DEVICE_CAP_GESTURE, preHandlerTime);
        if (ret != RET_OK) {
            return ret;
        }
        ret = RegisteredEventDispatch(ID_MSG_ON_SHOW_NOTIFICATION, udsServer, registeredEvent,
            INPUT_DEVICE_CAP_GESTURE, preHandlerTime);
    } else if (idMsg == ID_MSG_ON_NEXT) {
        ret = RegisteredEventDispatch(ID_MSG_ON_NEXT, udsServer, registeredEvent,
            INPUT_DEVICE_CAP_GESTURE, preHandlerTime);
        if (ret != RET_OK) {
            return ret;
        }
        ret = RegisteredEventDispatch(ID_MSG_ON_BACK, udsServer, registeredEvent,
            INPUT_DEVICE_CAP_GESTURE, preHandlerTime);
    } else {
...
int32_t OHOS::MMI::EventDispatch::RegisteredEventDispatch(const ID_MSG& idMsg, OHOS::MMI::UDSServer& udsServer,
    RegisteredEvent& registeredEvent, int32_t inputDeviceType, uint64_t preHandlerTime)
{
    CHKR(idMsg > ID_MSG_INVALID, PARAM_INPUT_INVALID, PARAM_INPUT_INVALID);
    ...
    auto retAutoTestMag = SendManagePktToAutoTest(udsServer, {}, WinMgr->GetFocusSurfaceId(), fds);
    ...
    for (auto fd : fds) {
        ...
        if (AppRegs->IsMultimodeInputReady(registeredEvent.occurredTime, idMsg, fd, testConnectState, testBufferState)) {
    ...
    libinput_event& event, int32_t inputDeviceType, uint64_t preHandlerTime)
{
    ...
    if ((key.key == KEY_ENTER || key.key == KEY_KPENTER) && key.state == KEY_STATE_PRESSED) {
        ret = RegisteredEventDispatch(ID_MSG_ON_SEND, udsServer, registeredEvent,
        ...
        ret = RegisteredEventDispatch(ID_MSG_ON_ENTER, udsServer, registeredEvent,
            INPUT_DEVICE_CAP_KEYBOARD, preHandlerTime);
        if (ret != RET_OK) {
            MMI_LOGE("Registered special Event dispatch failed return:%{public}d errCode:%{public}d", ret,
                SPCL_REG_EVENT_DISP_FAIL);
        ...
        ret = RegisteredEventDispatch(ID_MSG_ON_CANCEL, udsServer, registeredEvent,
            INPUT_DEVICE_CAP_KEYBOARD, preHandlerTime);
        if (ret != RET_OK) {
            MMI_LOGE("Registered special Event dispatch failed return:%{public}d errCode:%{public}d", ret,
                SPCL_REG_EVENT_DISP_FAIL);
        ...
        ret = RegisteredEventDispatch(ID_MSG_ON_CLOSEPAGE, udsServer, registeredEvent,
            INPUT_DEVICE_CAP_KEYBOARD, preHandlerTime);
        if (ret != RET_OK) {
            MMI_LOGE("Registered special Event dispatch failed return:%{public}d errCode:%{public}d", ret,
                SPCL_REG_EVENT_DISP_FAIL);
    ...
    EventTabletPad& tabletPad, const uint64_t preHandlerTime)
{
    ...
    auto retAutoTestMag = SendManagePktToAutoTest(udsServer, appInfo, WinMgr->GetFocusSurfaceId(), {});
    if (retAutoTestMag != RET_OK) {
        MMI_LOGE("Send event to auto-test failed! errCode:%{public}d", KEY_EVENT_DISP_FAIL);
    }

    const AutoTestDispatcherPkt autoTestDispatcherPkt = {
        "eventTabletPad", tabletPad.eventType, 0, 0,
        0, /* tabletPad.ring.position, */ 0, /* tabletPad.strip.position, */ 0,
...
#endif // DEBUG_CODE_TEST
    if (AppRegs->IsMultimodeInputReady(tabletPad.time, ID_MSG_ON_TOUCH, appInfo.fd,
        testConnectState, testBufferState)) {
        NetPacket newPacket(ID_MSG_ON_TOUCH);
    ...
    EventJoyStickAxis& eventJoyStickAxis, const uint64_t preHandlerTime)
{
    ...
    AutoTestDispatcherPkt autoTestDispatcherPkt = {
        "eventJoyStickAxis", eventJoyStickAxis.eventType, 0, 0, 0, 0, 0, appInfo.fd, focusId,
    ...
    int32_t testBufferState = 0;
#endif // DEBUG_CODE_TEST
    if (AppRegs->IsMultimodeInputReady(eventJoyStickAxis.time, ID_MSG_ON_TOUCH, appInfo.fd,
        testConnectState, testBufferState)) {
        NetPacket newPacket(ID_MSG_ON_TOUCH);
    ...
    EventTabletTool& tableTool, const uint64_t preHandlerTime, WindowSwitch& windowSwitch)
{
    ...
    if (AppRegs->IsMultimodeInputReady(tableTool.time, ID_MSG_ON_TOUCH, appInfo.fd, testConnectState, 
        testBufferState)) {
        NetPacket newPacket(ID_MSG_ON_TOUCH);
        ...
        const AutoTestDispatcherPkt autoTestDispatcherPkt = {
            ...
            inputEvent.x, inputEvent.y, 0, appInfo.fd, focusId, appInfo.abilityId,
    ...
    EventPointer &point, const uint64_t preHandlerTime, WindowSwitch& windowSwitch)
{
    ...
    int32_t testBufferState = 0;
#endif // DEBUG_CODE_TEST
    if (AppRegs->IsMultimodeInputReady(point.time, ID_MSG_ON_TOUCH, appInfo.fd, testConnectState, testBufferState)) {
        ...
        const AutoTestDispatcherPkt autoTestDispatcherPkt = {
            ...
            0, appInfo.fd, desWindowId, appInfo.abilityId, point.delta.x, point.delta.y,
    ...
    EventGesture& gesture, const uint64_t preHandlerTime)
{
    ...
    ID_MSG idMsg = ID_MSG_INVALID;
    MMIRegEvent->OnEventGestureGetSign(gesture, idMsg);
    if (idMsg != ID_MSG_INVALID) {
        RegisteredEvent registeredEvent = {};
    ...
    const AutoTestDispatcherPkt autoTestDispatcherPkt = {
        ...
        gesture.delta.x, gesture.delta.y, 0, appInfo.fd, focusId, appInfo.abilityId,
    ...
    int32_t testBufferState = 0;
#endif // DEBUG_CODE_TEST
    if (AppRegs->IsMultimodeInputReady(gesture.time, ID_MSG_ON_TOUCH, appInfo.fd, testConnectState, testBufferState)) {
        NetPacket newPacket(ID_MSG_ON_TOUCH);
...
#endif // DEBUG_CODE_TEST
    if (AppRegs->IsMultimodeInputReady(touch.time, ID_MSG_ON_TOUCH, appInfo.fd, testConnectState, testBufferState)) {
        NetPacket newPacket(ID_MSG_ON_TOUCH);
        ...
        if (!touchIds.empty()) {
            for (PAIR<uint32_t, int32_t> touchId : touchIds) {
                ...
                // Send event to auto-test frame
                const AutoTestDispatcherPkt autoTestDispatcherPkt = {
                    "eventTouch", touchTemp.eventType, 0, 0,
                    touchTemp.point.x, touchTemp.point.y, 0, appInfo.fd, touchFocusId, appInfo.abilityId,
                    0, 0, static_cast<uint16_t>(touchTemp.deviceType), touchTemp.deviceId, 0, touchTemp.slot
                };
                auto retAutoTestDpc = SendDispatcherPktToAutoTest(udsServer, autoTestDispatcherPkt);
                if (retAutoTestDpc != RET_OK) {
                    MMI_LOGE("Send event to auto-test failed! errCode:%{public}d", KEY_EVENT_DISP_FAIL);
        ...
        if (touch.eventType == LIBINPUT_EVENT_TOUCH_UP) {
            ...
            const AutoTestDispatcherPkt autoTestDispatcherPkt = {
                ...
                touch.point.x, touch.point.y, 0, appInfo.fd, touchFocusId, appInfo.abilityId,
    ...
    const KeyEventValueTransformations& trs, EventKeyboard& key, const uint64_t preHandlerTime)
{
    ...
    // Send event to auto-test frame
    AutoTestDispatcherPkt autoTestDispatcherPkt = {
        "eventKeyboard", key.eventType, key.key, key.state, 0, 0, 0, appInfo.fd, focusId,
...
#endif // DEBUG_CODE_TEST
    if (AppRegs->IsMultimodeInputReady(key.time, ID_MSG_ON_KEY, appInfo.fd, testConnectState, testBufferState)) {
        NetPacket newPkt(ID_MSG_ON_KEY);
    ...
    const AutoTestLibinputPkt& autoTestLibinputPkt)
{
    ...
    NetPacket pktAutoTest(ID_MSG_AUTOTEST_FRAME_LIBPKT);
...
int32_t OHOS::MMI::EventDispatch::SendMappingPktToAutoTest(UDSServer& udsServer, int32_t sourceType)
{
    ...
    NetPacket pktAutoTest(ID_MSG_AUTOTEST_FRAME_MAPPKT);
    ...
    const AutoTestStandardPkt& autoTestStandardPkt)
{
    ...
    NetPacket pktAutoTest(ID_MSG_AUTOTEST_FRAME_STDPKT);
    ...
    const AutoTestDispatcherPkt& autoTestDispatcherPkt) const
{
    ...
    NetPacket pktAutoTest(ID_MSG_AUTOTEST_FRAME_DPCPKT);
...
int32_t OHOS::MMI::EventDispatch::SendManagePktToAutoTest(UDSServer& udsServer, const OHOS::MMI::AppInfo& appInfo,
    const int32_t focusId, const std::vector<int32_t>& fds) const
{
    if (!AppRegs->AutoTestGetAutoTestFd()) {
        return RET_OK;
    }
    ...
    NetPacket pktAutoTest(ID_MSG_AUTOTEST_FRAME_MAGPKT);
    ...
    const AutoTestKeyTypePkt& autoTestKeyTypePkt)
{
    ...
    NetPacket pktAutoTest(ID_MSG_AUTOTEST_FRAME_KEYTYPEPKT);
