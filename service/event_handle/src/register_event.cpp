...
void RegisterEvent::OnEventKeyGetSign(EventKeyboard& key, ID_MSG& msg, EventKeyboard& prevKey)
{
...
bool RegisterEvent::OnGetRepeatKetState(const uint32_t keyCode, ID_MSG& msgId)
{
    EventHandle taskCode[] = {
        {KEY_SCREENRECORD, BIT4, ID_MSG_ON_STOP_SCREEN_RECORD},
        {KEY_VIDEO, BIT5, ID_MSG_ON_STOP_SCREEN_RECORD}
    };
    for (auto it : taskCode) {
...
int32_t RegisterEvent::OnEventKeyJudge(EventKeyboard& key, ID_MSG& msgId, EventKeyboard& prevKey)
{
    EventHandle eventHandle[] = {
        {KEY_SEARCH, 0, ID_MSG_ON_SEARCH}, {KEY_PAUSE, 0, ID_MSG_ON_PAUSE}, {KEY_PLAY, 0, ID_MSG_ON_PLAY},
        {KEY_CHANNELUP, 0, ID_MSG_ON_PREVIOUS}, {KEY_CHANNELDOWN, 0, ID_MSG_ON_NEXT},
        {KEY_VOLUMEUP, 0, ID_MSG_ON_PREVIOUS}, {KEY_VOLUMEDOWN, 0, ID_MSG_ON_NEXT},
        {KEY_MENU, 0, ID_MSG_ON_SHOW_MENU}, {KEY_ASSISTANT, 0, ID_MSG_ON_LAUNCH_VOICE_ASSISTANT},
        {KEY_F11, 0, ID_MSG_ON_SCREEN_SHOT}, {KEY_S, (GetBitNum(BIT0) + GetBitNum(BIT3)), ID_MSG_ON_SCREEN_SHOT},
        {KEY_S, GetBitNum(BIT0), ID_MSG_ON_SEARCH}, {KEY_D, GetBitNum(BIT0), ID_MSG_ON_GOTO_DESKTOP},
        {KEY_H, GetBitNum(BIT0), ID_MSG_ON_GOTO_DESKTOP}, {KEY_TAB, GetBitNum(BIT2), ID_MSG_ON_RECENT},
        {KEY_TAB, GetBitNum(BIT0), ID_MSG_ON_RECENT}, {KEY_N, GetBitNum(BIT0), ID_MSG_ON_SHOW_NOTIFICATION},
        {KEY_L, GetBitNum(BIT0), ID_MSG_ON_LOCK_SCREEN}, {KEY_F, GetBitNum(BIT1), ID_MSG_ON_SEARCH},
        {KEY_F4, GetBitNum(BIT2), ID_MSG_ON_CLOSEPAGE}, {KEY_F4, 0, ID_MSG_ON_MUTE},
        {KEY_W, GetBitNum(BIT1), ID_MSG_ON_CLOSEPAGE}, {KEY_PAGEUP, 0, ID_MSG_ON_PREVIOUS},
        {KEY_PAGEDOWN, 0, ID_MSG_ON_NEXT}, {KEY_R, GetBitNum(BIT1), ID_MSG_ON_REFRESH},
        {KEY_F5, 0, ID_MSG_ON_REFRESH}, {KEY_Z, GetBitNum(BIT1), ID_MSG_ON_UNDO},
        {KEY_X, GetBitNum(BIT1), ID_MSG_ON_CUT}, {KEY_C, GetBitNum(BIT1), ID_MSG_ON_COPY},
        {KEY_V, GetBitNum(BIT1), ID_MSG_ON_PASTE}, {KEY_F10, GetBitNum(BIT3), ID_MSG_ON_SHOW_MENU},
        {KEY_F10, GetBitNum(BIT0) + GetBitNum(BIT3), ID_MSG_ON_SHOW_MENU}, {KEY_MENU, 0, ID_MSG_ON_SHOW_MENU},
        {KEY_VIDEO, 0, ID_MSG_ON_START_SCREEN_RECORD}, {KEY_VIDEO, 0, ID_MSG_ON_STOP_SCREEN_RECORD},
        {KEY_DOWN, 0, ID_MSG_ON_NEXT}, {KEY_P, GetBitNum(BIT1), ID_MSG_ON_PRINT}, {KEY_UP, 0, ID_MSG_ON_PREVIOUS},
        {KEY_LEFT, 0, ID_MSG_ON_PREVIOUS}, {KEY_VOICECOMMAND, 0, ID_MSG_ON_LAUNCH_VOICE_ASSISTANT},
        {KEY_COMPOSE, 0, ID_MSG_ON_SHOW_MENU}, {KEY_RIGHT, 0, ID_MSG_ON_NEXT},
        {KEY_HOMEPAGE, 0, ID_MSG_ON_GOTO_DESKTOP}, {KEY_POWER, 0, ID_MSG_ON_LOCK_SCREEN},
        {KEY_SEARCH, 0, ID_MSG_ON_SEARCH}, {KEY_MULTITASK, 0, ID_MSG_ON_SEARCH},
        {KEY_SCREENSHOT, 0, ID_MSG_ON_SCREEN_SHOT}, {KEY_SCREENRECORD, 0, ID_MSG_ON_START_SCREEN_RECORD},
        {KEY_SCREENRECORD, 0, ID_MSG_ON_STOP_SCREEN_RECORD}, {KEY_RECENT, 0, ID_MSG_ON_RECENT},
        {KEY_NOTIFICATION, 0, ID_MSG_ON_SHOW_NOTIFICATION},
    };
...
int32_t RegisterEvent::OnEventPointButton(const int32_t buttonCode, const uint64_t timeNow,
                                          const BUTTON_STATE stateValue, ID_MSG& msgId)
{
    ...
    if (buttonCode == BTN_MIDDLE && stateValue == BUTTON_STATE_RELEASED) {
        if (timeCount_ == 0) {
            ...
            if (timeNow - timeCount_ >= (INTERVALTIME / BIT2)) {
                msgId = ID_MSG_ON_GOTO_DESKTOP;
                timeCount_ = 0;
            } else {
                msgId = ID_MSG_ON_BACK;
            }
        }
    }
    if (buttonCode == BTN_RIGHT && stateValue == BUTTON_STATE_PRESSED) {
        msgId = ID_MSG_ON_SHOW_MENU;
    }
    return RET_OK;
}

int32_t RegisterEvent::OnEventPointAxis(const EventPointer& point, ID_MSG& msgId)
{
    if (point.axes == POINTER_AXIS_SCROLL_VERTICAL && point.delta.y < 0) {
        msgId = ID_MSG_ON_PREVIOUS;
    }
    if (point.axes == POINTER_AXIS_SCROLL_VERTICAL && point.delta.y > 0) {
        msgId = ID_MSG_ON_NEXT;
    }
    return RET_OK;
}

void RegisterEvent::OnEventGestureGetSign(const EventGesture& gesture, ID_MSG& msgId)
{
...
void RegisterEvent::OnEventTouchGetSign(const EventTouch& touch, ID_MSG& msgId)
{
...
int32_t RegisterEvent::OnEventGestureUpdateGetSign(const EventGesture& gesture, ID_MSG& msgId)
{
...
int32_t RegisterEvent::OnEventGestureEndGetSign(const EventGesture& gesture, ID_MSG& msgId)
{
...
int32_t RegisterEvent::OnEventOneFingerHandlerGetSign(ID_MSG& msgId, TouchInfo& touchUpInfo)
{
...
int32_t RegisterEvent::OnEventThreeFingerHandlerGetSign(ID_MSG& msgId, TouchInfo& touchUpInfo)
{
    ...
    if (touchState) {
        msgId = ID_MSG_ON_SCREEN_SHOT;
    }
    return RET_OK;
}

int32_t RegisterEvent::OnEventTouchUpGetSign(const EventTouch& touch, ID_MSG& msgId)
{
    CHKF(touch.time > 0, PARAM_INPUT_INVALID);
    CHKF(touch.seat_slot >= 0, PARAM_INPUT_INVALID);
...
int32_t RegisterEvent::OnEventTouchMotionGetSign(const EventTouch& touch, ID_MSG& msgId)
{
    CHKF(touch.time > 0, PARAM_INPUT_INVALID);
    CHKF(touch.seat_slot >= 0, PARAM_INPUT_INVALID);
    auto iter = touchInfos_.find(MAKEPAIR(touch.deviceId, touch.seat_slot));
    ...
    if (GetTouchInfoSizeByDeviceId(touch.deviceId) == ONEFINGER) {
        if ((iter->second.beginY >= MINY) && (iter->second.beginY < MINY + REGION) && (iter->second.beginX != MINX) &&
            ...
            msgId = ID_MSG_ON_SHOW_NOTIFICATION;
            return RET_OK;
        }
