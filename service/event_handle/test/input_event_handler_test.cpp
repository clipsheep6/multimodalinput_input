...
const static libinput_interface LIBINPUT_INTERFACE = {
    ...
    {
        printf("libinput .close_restricted fd:%d\n", fd);
        close(fd);
    },
};
...
public:
    ...
    void SetUp()
    {
        ...
        udev_ = udev_new();
        EXPECT_TRUE(udev_);
        input_ = libinput_udev_create_context(&LIBINPUT_INTERFACE, NULL, udev_);
        EXPECT_TRUE(input_);
        ...
        if (virtualDeviceManagerPid_ == 0) {
            execl(DEF_MMI_VIRTUAL_DEVICE_MANAGER, "hosmmi-virtual-device-manager", "start", currentArgsName_.c_str(), NULL);
        } else {
            usleep(delayTime);
            LibinputDrainEvents(input_);
            eventinjectionPid_ = fork();
            if (eventinjectionPid_ == 0) {
                execl(DEF_MMI_EVENT_INJECTION, "hosmmi-event-injection", "json", currentJsonName_.c_str(), NULL);
            } else {
                waitpid(eventinjectionPid_, NULL, 0);
    ...
    void TearDown()
    {
        ...
        waitpid(virtualDeviceManagerPid_, NULL, 0);
