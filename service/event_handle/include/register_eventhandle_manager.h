...
public:
    ...
    int32_t RegisterEvent(int32_t eventHangle, int32_t fd);

    int32_t UnregisterEventHandleManager(int32_t eventHangle, int32_t fd);

    void UnregisterEventHandleBySocketFd(int32_t fd);

    int32_t FindSocketFdsByEventHandle(const int32_t eventHangle, std::vector<int32_t>& fds);
...
private:
    std::mutex mu_;
    std::multimap<int32_t, int32_t> mapRegisterManager_ = {}; // key=enum ID_MSG : value=fd
};
}
}
