...
public:
    ...
    */
    void OnEventKeyGetSign(EventKeyboard& key, ID_MSG& msg, EventKeyboard& prevKey);
    int32_t OnEventPointButton(const int32_t buttonCode, const uint64_t timeNow,
        const BUTTON_STATE stateValue, ID_MSG& msgId);
    int32_t OnEventPointAxis(const EventPointer& point, ID_MSG& msgId);
    /*
    ...
    */
    void OnEventTouchGetSign(const EventTouch& touch, ID_MSG& msgId);
    void OnEventGestureGetSign(const EventGesture& gesture, ID_MSG& msgId);
...
protected:
    int32_t OnEventGestureEndGetSign(const EventGesture& gesture, ID_MSG& msgId);
    int32_t OnEventGestureUpdateGetSign(const EventGesture& gesture, ID_MSG& msgId);
    int32_t OnEventGestureBeginGetSign(const EventGesture& gesture);
    int32_t OnEventOneFingerHandlerGetSign(ID_MSG& msgId, TouchInfo& touchUpInfo);
    int32_t OnEventThreeFingerHandlerGetSign(ID_MSG& msgId, TouchInfo& touchUpInfo);
    int32_t OnEventTouchDownGetSign(const EventTouch& touch);
    int32_t OnEventTouchMotionGetSign(const EventTouch& touch, ID_MSG& msgId);
    int32_t OnEventTouchUpGetSign(const EventTouch& touch, ID_MSG& msgId);
    bool OnGetRepeatKetState(const uint32_t keyCode, ID_MSG& msgId);
    int32_t OnEventKeyJudge(EventKeyboard& key, ID_MSG& msgId, EventKeyboard& prevKey);
    /*
    * Method:    GetBitNum
