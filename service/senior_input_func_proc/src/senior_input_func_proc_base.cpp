...
{
    const int16_t msgId = static_cast<int16_t>(event.ev_code);
    const uint32_t occurredTime = static_cast<uint32_t>(event.stamp);
    const std::string uuid = GetUUid();

    if (msgId == ID_MSG_INVALID) {
    ...
    for (auto fd : fds) {
        ...
        const AutoTestDispatcherPkt autoTestDispatcherPkt = {
            "eventAi_Knuckle", static_cast<uint16_t>(event.ev_type), event.ev_code, 0, 0, 0, 0, 
        ...
        MMI_LOGI("senior input func process server: fd = %{public}d,windowId = %{public}d,abilityId = %{public}d,"
                 "conbinecode = %{public}d", fd, appInfo.windowId, appInfo.abilityId, event.ev_code);
    }
...
int32_t SeniorInputFuncProcBase::ReplyMessage(SessionPtr aiSessionPtr, int32_t status)
{
    ...
    NetPacket newPacket(ID_MSG_SENIOR_INPUT_FUNC);
    newPacket << status;
