    ...
    for (auto iter = subscriberMap_.begin(); iter != subscriberMap_.end(); iter++) {
        ...
        for (auto it = subscribers.begin(); it != subscribers.end(); it++) {
            if ((*it)->id_ == subscribeId && (*it)->sess_ == sess) {
                ...
                MMI_HILOGI("subscribers size: %{public}lu, subscribeId: %{public}d", subscribers.size(), subscribeId);
                return RET_OK;
            }
        }
    }
    ...
    std::shared_ptr<KeyOption> option)
{
    ...
    for (auto &iter : subscriberMap_) {
        if (IsEqualKeyOption(option, iter.first)) {
            ...
            MMI_HILOGI("subscriber size: %{public}lu", iter.second.size());
            return;
        }
    }
    MMI_HILOGI("add subscriber Id: %{public}d", subscriber->id_);
...
{
    CALL_DEBUG_ENTER;
    std::vector<AppExecFwk::AppStateData> list = AppObserverMgr->GetForegroundAppData();
    ...
    std::set<int32_t> foregroundPids)
{
    ...
    MMI_HILOGD("isForegroundExits_: %{public}d, foregroundPids: %{public}lu",
    ...
    std::shared_ptr<KeyOption> keyOption, std::list<std::shared_ptr<Subscriber>> &subscribers, bool &handled)
{
    ...
    CHKPV(keyEvent);
    CHKPV(keyOption);
    MMI_HILOGI("notify key down subscribers size: %{public}lu", subscribers.size());
...
{
    CALL_DEBUG_ENTER;
    MMI_HILOGD("subscribers size: %{public}lu", subscribers.size());
    for (auto &subscriber : subscribers) {
        CHKPC(subscriber);
        auto sess = subscriber->sess_;
        CHKPC(sess);
...
{
    CALL_DEBUG_ENTER;
    MMI_HILOGD("clear subscriber timer size: %{public}lu", subscribers.size());
    for (auto &subscriber : subscribers) {
        CHKPC(subscriber);
...
bool KeySubscriberHandler::HandleKeyDown(const std::shared_ptr<KeyEvent> &keyEvent)
{
    ...
    MMI_HILOGE("foreground pid size: %{public}lu", pids.size());
    for (auto &iter : subscriberMap_) {
...
bool KeySubscriberHandler::HandleKeyUp(const std::shared_ptr<KeyEvent> &keyEvent)
{
    ...
    std::set<int32_t> pids;
    GetForegroundPids(pids);
    MMI_HILOGD("foreground pid size: %{public}lu", pids.size());
    for (auto &iter : subscriberMap_) {
        auto keyOption = iter.first;
        auto subscribers = iter.second;
        PrintKeyOption(keyOption);
