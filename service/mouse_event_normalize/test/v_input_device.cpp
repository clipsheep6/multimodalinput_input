...
int32_t VInputDevice::Open()
{
    ...
    if (realpath(devPath_.c_str(), buf) == nullptr) {
        std::cout << "Not real path: " << devPath_ << std::endl;
        return RET_ERR;
    }
    ...
    for (;;) {
        ...
        if (fd_ < 0) {
            std::cout << "Unable to open device '" << buf << "': " << buf << ::strerror(errno) << std::endl;
            if (nRetries-- > 0) {
                std::this_thread::sleep_for(std::chrono::milliseconds(SLEEP_TIME));
                std::cout << "Retry opening device \'" << buf << " \'" << std::endl;
            } else {
                return RET_ERR;
            }
        } else {
            std::cout << "Opening \'" << buf << "\' successfully" << std::endl;
            break;
        }
    }
    QueryDeviceInfo();
