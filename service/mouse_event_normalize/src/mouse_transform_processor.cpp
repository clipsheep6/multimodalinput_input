...
int32_t MouseTransformProcessor::HandleButtonInner(struct libinput_event_pointer* data, struct libinput_event *event)
{
    ...
    const int32_t type = libinput_event_get_type(event);
    bool tpLeftSwitch = true;
    if (GetTouchpadTapSwitch(tpLeftSwitch) != RET_OK) {
        MMI_HILOGD("Failed to get touch pad left switch flag, default is true.");
    }

    // touch pad tap switch is disable
    if (type == LIBINPUT_EVENT_POINTER_TAP && tpLeftSwitch == false) {
...
int32_t MouseTransformProcessor::HandleAxisInner(struct libinput_event_pointer* data)
{
    ...
    MMI_HILOGD("lisenhao libinput_pointer_axis_source = :%{public}d", source);
    bool tpScrollSwitch = false;
    if (GetTouchpadScrollSwitch(tpScrollSwitch) != RET_OK) {
        MMI_HILOGD("Failed to get scroll switch flag, default is true.");
    }
    ...
    int32_t scrollDirect = 1;
    bool scrollDirectSwitch = false;
    if (GetTouchpadScrollDirection(scrollDirectSwitch) != RET_OK) {
        MMI_HILOGD("Failed to get scroll direct switch flag, default is true.");
    }
    if (scrollDirectSwitch == false && source == LIBINPUT_POINTER_AXIS_SOURCE_FINGER) {
        scrollDirect = -1;
    }
    ...
    if (libinput_event_pointer_has_axis(data, LIBINPUT_POINTER_AXIS_SCROLL_VERTICAL)) {
        ...
        axisValue = GetMouseScrollRows() * (axisValue / initRows) * scrollDirect;
