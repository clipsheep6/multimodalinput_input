...
void OHOS::MMI::AbilityLaunchManager::ResolveConfig(std::string configFile)
{
    if (!FileExists(configFile)) {
        MMI_LOG_E("ability config file %{public}s not exist!", configFile.c_str());
        return;
    }
    MMI_LOG_E("ability config file path %{public}s", configFile.c_str());
    std::ifstream reader(configFile);
    if (!reader.is_open()) {
        MMI_LOG_E("json file can not open!");
        return;
    }
    json configJson;
    reader >> configJson;
    ...
    if (!abilityArray.is_array() || abilityArray.empty()) {
        MMI_LOG_E("shortkeys array is empty!");
        return;
    }
...
bool OHOS::MMI::AbilityLaunchManager::ConvertJson(ShortcutKey &shortcutKey, json &jsonData)
{
    ...
    if (!preKey.is_array() || preKey.size() != 4) {
        MMI_LOG_E("preKey array length must be four!");
        return false;
    }

    if (!preKey[0].is_number() || !preKey[1].is_number() || !preKey[2].is_number() || !preKey[3].is_number()) {
        MMI_LOG_E("preKey type must be number!");
        return false;
    }
    ...
    if (!jsonData["finalKey"].is_number()) {
        MMI_LOG_E("finalKey type must be number!");
        return false;
    }
    ...
    if (!jsonData["trigger"].is_string() ||
        ...
        MMI_LOG_E("trigger must be one of [key_up,key_down]!");
        return false;
    }
...
bool OHOS::MMI::AbilityLaunchManager::UnwrapAbility(Ability &ability, json &jsonAbility)
{
    if (!jsonAbility.is_object() || !jsonAbility["entities"].is_array()) {
        MMI_LOG_E("ability type must be object!");
        return false;
    }
...
void OHOS::MMI::AbilityLaunchManager::PrintShortcutKey()
{
    ...
    MMI_LOG_E("shortcutKeysMap size %{public}d", count);
    for (auto it = shortcutKeysMap.begin(); it != shortcutKeysMap.end(); ++it) {
        auto &shortcutKey = it->second;
        MMI_LOG_E("preKey1 = %{public}d preKey2 = %{public}d preKey3 = %{public}d preKey4 = %{public}d finalKey = %{public}d "
...
bool OHOS::MMI::AbilityLaunchManager::CheckLaunchAbility(std::shared_ptr<OHOS::MMI::KeyEvent> &key)
{
    if (CheckShortcutkeyMatch(waitTriggerKey, key)) {
        MMI_LOG_E("The same shortcutkey is waiting timeout");
        return true;
    }
    timer.Stop();
    ...
    for (auto iter = shortcutKeysMap.begin(); iter != shortcutKeysMap.end(); ++iter) {
        ...
        if (shortcutKey.triggerType == OHOS::MMI::KeyEvent::KEY_ACTION_DOWN && shortcutKey.keyDownDuration > 0) {
            MMI_LOG_E("Key event matched, start Timer, key=%{public}d, keyAction=%{public}d", key->GetKeyCode(), key->GetKeyAction());
            ...
            MMI_LOG_E("Start launch ability");
...
void OHOS::MMI::AbilityLaunchManager::LaunchAbility(ShortcutKey key)
{
    ...
    MMI_LOG_E("Start launch ability, abilityName:%{public}s", key.ability.abilityName.c_str());
    ErrCode err = AAFwk::AbilityManagerClient::GetInstance()->StartAbility(want);
    if (err != ERR_OK) {
        MMI_LOG_E("LaunchAbility failed, abilityName:%{public}s, err:%{public}d", key.ability.abilityName.c_str(), err);
    }
    ResetWaitTriggerKey(waitTriggerKey);
    MMI_LOG_E("End launch ability, abilityName:%{public}s", key.ability.abilityName.c_str());
}
        ...
        if (time == 0) {
            ...
            if (condition.wait_for(lk, std::chrono::milliseconds(time)) == std::cv_status::timeout) {
                if (callback_ != nullptr) {
                    MMI_LOG_E("timeout, start launch ability abilityName %{public}s", shortcutKey.ability.abilityName.c_str());
