...
constexpr double MAXIMUM_SINGLE_SLIDE_DISTANCE = 0.3f;
constexpr double MINIMUM_GRAVITY_OFFSET = 0.1f;
...
void TouchGestureDetector::HandleDownEvent(std::shared_ptr<PointerEvent> event)
{
    ...
    if (!iter.second) {
        MMI_HILOGE("Insert value failed, pointerId:%{public}d", pointerId);
    }
    if (getureType_ == PinchAdapterType) {
        CalcAndStoreDistance(downPoint_);
    } else if (getureType_ == SwipeAdapterType) {
        isFingerReady_ = HandleFingerDown();
    }
    MMI_HILOGI("getureType:%{public}d, finger count:%{public}zu, isFingerReady:%{public}s,"
        "pointerId:%{public}d, x:%{public}d, y:%{public}d, time:%{public}ld",
        getureType_, downPoint_.size(), isFingerReady_ ? "true" : " false", pointerId, x, y, time);
}
...
void TouchGestureDetector::HandlePinchMoveEvent(std::shared_ptr<PointerEvent> event)
{
    ...
    std::map<int32_t, Point> movePoint;
    for (const auto &[pointerId, _] : downPoint_) {
        ...
        int32_t y = item.GetDisplayY();
        int64_t time = item.GetDownTime();
        auto iter = movePoint.insert(std::make_pair(pointerId, Point(x, y, time)));
        if (!iter.second) {
            MMI_HILOGE("Insert value failed, pointerId:%{public}d", pointerId);
        }
    }
    if (CalcMultiFingerMovement(movePoint) > downPoint_.size() - MINIMUM_FINGER_COUNT_OFFSET) {
        GetureType type = JudgeOperationMode(movePoint);
...
{
    int fingersCount = downPoint_.size();
...
int64_t TouchGestureDetector::GetMaxDownInterval()
{
    ...
    MMI_HILOGI("Down interval: %{public}ld", (latestTime - earliestTime));
    return latestTime - earliestTime;
}
...
TouchGestureDetector::SlideState TouchGestureDetector::ClacFingerMoveDirection(std::shared_ptr<PointerEvent> event)
{
    ...
    for (const auto &dPoint : downPoint_) {
        ...
        MMI_HILOGI("pointerId: %{public}d, point.x:%{public}d, point.y:%{public}d, x:%{public}d, y:%{public}d, isMove:%{public}s",
            pointerId, point.x, point.y, x, y, isMove ? "true" : "false");
        if (fabs(angle) < FLT_EPSILON && !isMove) {
            MMI_HILOGW("angle is zero.");
            continue;
        }
        ...
        MMI_HILOGI("id: %{public}d, angle: %{public}.2f, direction: %{public}d", pointerId, angle, direction);
    }
    if (recognizedCount < THREE_FINGER_COUNT || directions.size() > 1) {
        return state;
    }
    return *(directions.begin());
}

double TouchGestureDetector::CalcTwoPointsDistance(Point p1, Point p2)
{
    return sqrt(labs((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y)));
}

Point TouchGestureDetector::CalcGravityCenter(std::map<int32_t, Point> map)
{
    double xSum = 0.0;
    double ySum = 0.0;
    ...
    delete[] vertices;
    return Point((int32_t)xSum, (int32_t)ySum);
}

void TouchGestureDetector::CalcAndStoreDistance(std::map<int32_t, Point> map)
{
    ...
    Point center = CalcGravityCenter(map);
    if (!lastDistance_.empty()) {
        lastDistance_.clear();
    }
...
int32_t TouchGestureDetector::CalcMultiFingerMovement(std::map<int32_t, Point> map)
{
    int32_t movementCount = 0;
    for (const auto &[id, point] : downPoint_) {
        auto movePoint = map.find(id);
        if (movePoint == map.end()) {
            return 0;
        }
        if (CalcTwoPointsDistance(point, movePoint->second) > MAXIMUM_SINGLE_SLIDE_DISTANCE) {
            ++movementCount;
        }
    }
