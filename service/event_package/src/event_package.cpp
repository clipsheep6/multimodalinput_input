...
int32_t EventPackage::PackageEventDeviceInfo(libinput_event *event, UDSServer& udsServer, EventType& eventData)
{
    ...
    if (type == LIBINPUT_EVENT_DEVICE_REMOVED) {
        GetDeviceRegister().DeleteDeviceInfo(devicePhys);
        return RET_OK;
    }
    uint32_t deviceId = GetDeviceRegister().FindDeviceIdByDevicePhys(devicePhys);
    if (deviceId) {
        eventData.deviceId = deviceId;
    } else {
        deviceId = GetDeviceRegister().AddDeviceInfo(devicePhys);
