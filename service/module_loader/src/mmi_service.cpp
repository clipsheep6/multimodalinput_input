...
#ifdef OHOS_BUILD_ENABLE_COOPERATE
    int32_t pid = GetCallingPid();
    int32_t ret = delegateTasks_.PostSyncTask(std::bind(&MMIService::OnEnableInputDeviceCooperate, this, pid, userData, enabled));
    if (ret != RET_OK) {
...
int32_t MMIService::StartInputDeviceCooperate(int32_t userData, const std::string &sinkDeviceId, int32_t srcInputDeviceId)
{
    CALL_DEBUG_ENTER;
#ifdef OHOS_BUILD_ENABLE_COOPERATE
    int32_t pid = GetCallingPid();
    int32_t ret = delegateTasks_.PostSyncTask(std::bind(&MMIService::OnStartInputDeviceCooperate, this, pid, userData, sinkDeviceId, srcInputDeviceId));
    if (ret != RET_OK) {
...
#ifdef OHOS_BUILD_ENABLE_COOPERATE
    int32_t pid = GetCallingPid();
    int32_t ret = delegateTasks_.PostSyncTask(std::bind(&MMIService::OnGetInputDeviceCooperateState, this, pid, userData, deviceId));
    if (ret != RET_OK) {
