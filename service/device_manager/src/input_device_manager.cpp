...
std::shared_ptr<InputDevice> InputDeviceManager::MakeInputDevice(int32_t id, struct libinput_device* libinputDevice) const
{
...
void InputDeviceManager::NotifyDeviceChanged(const std::string& deviceId, const uint32_t& inputTypes, std::string changedType)
{
    CALL_LOG_ENTER;
    for (const auto item : inputDevice_) {
        if (!IsRemote(item.second)) {
