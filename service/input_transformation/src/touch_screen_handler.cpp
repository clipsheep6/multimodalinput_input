...
void TouchScreenHandler::ResetTouchUpEvent(std::shared_ptr<PointerEvent> pointerEvent, const std::shared_ptr<AbsEvent> event)
{
    CHKPV(pointerEvent);
    CHKPV(event);
    if (event->GetAction() == AbsEvent::ACTION_UP) {
