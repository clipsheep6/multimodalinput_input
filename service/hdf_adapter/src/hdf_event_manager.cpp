...
int OHOS::MMI::HdfEventManager::GetDeviceCount()
{
    int jectcount = 0;
    int devcount = 0;
    memset_s(mountDevIndex_, sizeof(DevDesc) * TOTAL_INPUT_DEVICE_COUNT, 0, sizeof(DevDesc) * TOTAL_INPUT_DEVICE_COUNT);
...
void OHOS::MMI::HdfEventManager::SetupCallback()
{
    ...
    if (ret != 0 || inputInterface_ == NULL
        || inputInterface_->iInputManager == NULL
        || inputInterface_->iInputReporter == NULL) {
        MMI_LOGD("---- %{public}s:%{public}d inputInterface_ init fail! ----\n", __func__, __LINE__);
    }
// #if 0
    ret = GetInputInterfaceFromInject(&injectInterface_);
    if (ret != 0 || injectInterface_ == NULL
        || injectInterface_->iInputManager == NULL
        || injectInterface_->iInputReporter == NULL) {
...
void OHOS::MMI::HdfEventManager::GetEventCallback(const EventPackage **pkgs, uint32_t count, uint32_t devIndex)
{
    ...
    for (uint32_t i = 0; i < count && i < MAX_EVENT_PKG_NUM; i++) {
        ...
        eventarry[i].type = (pkgs[i]->type) | (uint16_t)(devIndex<<8); // 不改变livinput结构传递，对象的index参数
...
{
    if (m_globleThis->hdiinput_ == NULL) {
        m_globleThis->hdiinput_ = libinput_hdf_create_context(&_hdfinterface, NULL);
    }
