...
class MMITestAuxTool : public UDSClient {
    using AiSensorData = std::vector<std::string>;
public:
    MMITestAuxTool() = default;
    ~MMITestAuxTool() = default;
    bool Start();
    void SetFunction(const ConnnectFun &fun);
...
protected:
    ...
    ConnnectFun fun_ = nullptr;
    };
}
using InjectToolClient = OHOS::MMI::CSingleton<OHOS::MMI::MMITestAuxTool>;
#endif