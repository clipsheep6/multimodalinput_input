...
HWTEST_F(ProcessingPenDeviceTest, Test_TransformJsonDataToInputData, TestSize.Level1)
{
    const string path = "temp/Test_TransformPenJsonDataToInputData.json";
#ifdef OHOS_BUILD
    string cmd = "hosmmi-virtual-device-manager start touchpad & ";
#else
    string cmd = "./hosmmi-virtual-deviced.out start touchpad &";
#endif
    system(cmd.c_str());
