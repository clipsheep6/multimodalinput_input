...
{
    MMI_LOG_I("Enter TransformJsonDataForGamePad function.");
    if (originalEvent.empty()) {
        return RET_ERR;
    }
    if (originalEvent.find("events") == originalEvent.end()) {
        MMI_LOG_E("manage game pad array faild, inputData is empty.");
        return RET_ERR;
    }
    ...
    MMI_LOG_I("Leave TransformJsonDataForTouchPad function.");
    ...
    for (auto item : inputData) {
        ...
        if ((eventType == "KEY_EVENT_CLICK") || (eventType == "KEY_EVENT_PRESS") ||
            ...
            if ((item.find("keyValue")) == item.end()) {
                MMI_LOG_E("function AnalysisGamePadEvent not find keyValue On Event: %{public}s.", eventType.c_str());
                return RET_ERR;
            }
            padEvent.keyValue = item.at("keyValue").get<int32_t>();
        } else if ((eventType == "ROCKER_1") || (eventType == "ROCKER_2")) {
            if ((item.find("event")) == item.end()) {
                MMI_LOG_E("function AnalysisGamePadEvent not find event On Event: %{public}s.", eventType.c_str());
                return RET_ERR;
            }
            if ((item.find("direction")) == item.end()) {
                MMI_LOG_E("function AnalysisGamePadEvent not find direction On Event: %{public}s.", eventType.c_str());
                return RET_ERR;
            }
