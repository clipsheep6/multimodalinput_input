...
int32_t GetDeviceNode::GetDeviceNodeByName(const string &targetName, string &deviceNode, uint16_t devIndex)
{
    ...
    if (iter == deviceMapData.end()) {
        MMI_LOG_E("GetDeviceNodeByName faild for find deviceName: %{public}s.", deviceName.c_str());
        return RET_ERR;
    }
    int32_t targetSize = static_cast<int32_t>(iter->second.size());
    if (devIndex > targetSize) {
        MMI_LOG_E("GetDeviceNodeByName faild for devIndex:%{public}d > targetSize:%{public}d.", devIndex, targetSize);
        return RET_ERR;
    }
    string nodeRootPath = "/dev/input/";
    deviceNode = nodeRootPath + iter->second[devIndex];
    MMI_LOG_I("GetDeviceNodeByName: %{public}s[%{public}d] --> %{public}s.", targetName.c_str(), devIndex,
             deviceNode.c_str());
