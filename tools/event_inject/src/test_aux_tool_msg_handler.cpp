...
{
    MsgCallback funs[] = {
        {ID_MSG_SENIOR_INPUT_FUNC, std::bind(&TestAuxToolMsgHandler::OnAiServerReply, 
...
void TestAuxToolMsgHandler::OnMsgHandler(const UDSClient &client, NetPacket &pkt)
{
    ...
    TimeCostChkfp1("TestAuxToolMsgHandler::OnMsgHandler", "overtime 200(us)", CHECK_TIME, id);
    auto fun = GetFun(id);
    if (!fun) {
...
int32_t TestAuxToolMsgHandler::OnAiServerReply(const UDSClient &client, NetPacket &pkt)
{
    int32_t replyCode = 0;
    pkt >> replyCode;
