...
int32_t InjectionEventDispatch::VerifyFile(const std::string strFileName)
{
    std::string findcmd = "find /data -name " + strFileName;
...
{
    char Path[PATH_MAX] = {};
    if (realpath(strFileName.c_str(), Path) == nullptr) {
        MMI_LOGE("json path is error, path:%{public}s", strFileName.c_str());
        return RET_ERR;
    }
    if (VerifyFile(strFileName)) {
        return RET_ERR;
    }
...
int32_t InjectionEventDispatch::OnJson()
{
    ...
    if ((fileSize <= 0) || (fileSize > JSON_FILE_SIZE)) {
        MMI_LOGE("The file size is out of range 2M or empty. filesize:%{public}lld", fileSize);
        return RET_ERR;
    }
