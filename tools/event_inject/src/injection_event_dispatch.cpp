...
int32_t InjectionEventDispatch::ExistFile(const std::string& fileName)
{
...
int32_t InjectionEventDispatch::OnJson()
{
    ...
    if (realpath(jsonFile.c_str(), Path) == nullptr) {
        MMI_LOGE("json path is error, path:%{public}s", jsonFile.c_str());
        return RET_ERR;
    }
    if (ExistFile(jsonFile)) {
        MMI_LOGE("This file does not exist.");
        return RET_ERR;
    }
    if (VerifyFile(jsonFile)) {
        MMI_LOGE("This file is not in /data.");
        return RET_ERR;
    }
