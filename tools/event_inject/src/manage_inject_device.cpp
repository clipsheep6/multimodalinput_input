...
{
    MMI_LOG_I("Enter TransformJsonData function.");
    if (configData.empty()) {
        MMI_LOG_E("input data from json file is empty.");
        return RET_ERR;
    }
    int ret = RET_ERR;
    string deviceName;
    ...
    MMI_LOG_I("Leave TransformJsonData function.");
...
{
    MMI_LOG_I("Enter sendEventToDeviveNode function.");
    string deviceNode = inputEventArray.target;
    if (deviceNode.empty()) {
        MMI_LOG_E("device node:%{public}s is not exit.", deviceNode.c_str());
        return RET_ERR;
    }
    int32_t fd = open(deviceNode.c_str(), O_RDWR);
    if (fd < 0) {
        MMI_LOG_E("open device node:%{public}s faild.", deviceNode.c_str());
        return RET_ERR;
    }
    ...
    MMI_LOG_I("Leave sendEventToDeviveNode function.");
    return RET_OK;
}