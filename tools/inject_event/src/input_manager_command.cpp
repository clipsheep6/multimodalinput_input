...
struct JoystickInfo {
    ...
    int32_t absValue { -1 };
    int32_t taktTime { 0 };
    PointerEvent::AbsoluteType absType;
};
} // namespace
...
int32_t InputManagerCommand::ParseCommand(int32_t argc, char *argv[])
{
    ...
    struct option joystickSensorOption[] = {
        {"move", required_argument, NULL, 'm'},
        {"down", required_argument, NULL, 'd'},
        {"up", required_argument, NULL, 'u'},
        {"click", required_argument, NULL, 'c'},
    ...
    if ((c = getopt_long(argc, argv, "MKTJ?", headOptions, &optionIndex)) != -1) {
        switch (c) {
            ...
            case 'J': {
                ...
                while ((c = getopt_long(argc, argv, "m:d:u:c:i:", joystickSensorOption, &optionIndex)) != -1) {
                    switch (c) {
                        case 'm': {
                            ...
                            if (absAction == "x") {
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_X;
                            } else if (absAction == "y") {
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_Y;
                            } else if (absAction == "z") {
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_Z;
                            } else if (absAction == "rz") {
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_Y;
                            } else if (absAction == "gas") {
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_GAS;
                            } else if (absAction == "brake") {
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_BRAKE;
                            } else if (absAction == "hat0x") {
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_HAT0X;
                            } else if (absAction == "hat0y"){
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_HAT0Y;
                            } else if (absAction == "throttle") {
                                joyInfo.absType = PointerEvent::AbsoluteType::ABSOLUTE_TYPE_THROTTLE;
                            } else {
                                std::cout << "The invalid abstype" << std::endl;
                                return RET_ERR;
                            }
                ...
                if (pointerEvent != nullptr) {
                    ...
                    if (state.empty()) {
                        std::cout << "Injection of failed" << std::endl;
                        return EVENT_REG_FAIL;
                    }
                    for (const auto &it : state) {
                        if (it.first == JOYSTICK_BUTTON_PRESS) {
                            ...
                            pointerEvent->SetPointerAction(PointerEvent::POINTER_ACTION_MOVE);
                            pointerEvent->SetAbsValue(it.second.absType, it.second.absValue);
