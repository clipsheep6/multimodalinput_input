...
int32_t InputManagerCommand::ProcessPinchGesture(int32_t argc, char *argv[], int32_t indexPercent)
{
    ...
    constexpr int32_t MAX_ARGC_INDEX = 4;
    if (indexPercent != MAX_ARGC_INDEX) {
        std::cout << "wrong optind pointer index" << std::endl;
        return EVENT_REG_FAIL;
    }
    ...
    constexpr int32_t MIN_SCALE_NUMERATOR = 0;
    constexpr int32_t MAX_SCALE_NUMERATOR = 500;
    if ((scalePercentNumerator <= MIN_SCALE_NUMERATOR) || (scalePercentNumerator > MAX_SCALE_NUMERATOR)) {
        std::cout << "Invalid scale numberator:" << scalePercentNumerator << std::endl;
        std::cout << std::endl;
        return RET_ERR;
    }
    int32_t ret = InjectPinchEvent(fingerCount, scalePercentNumerator);
    if (ret != ERR_OK) {
        return ret;
    }
    return ERR_OK;
