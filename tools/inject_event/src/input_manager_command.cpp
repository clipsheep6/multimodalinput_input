                ...
                while ((c = getopt_long(argc, argv, "m:d:u:c:s:", mouseSensorOptions, &optionIndex)) != -1) {
                    switch (c) {
                        ...
                        case 's': {
                            ...
                            pointerEvent->SetAxisValue(MMI::PointerEvent::AxisType::AXIS_TYPE_SCROLL_VERTICAL,scrollValue);
                            pointerEvent->SetSourceType(MMI::PointerEvent::SOURCE_TYPE_MOUSE);
                            MMI::InputManager::GetInstance()->SimulateInputEvent(pointerEvent);
                            break;
                        }
                    ...
                    switch (c) {
                        case 'm': {
                            ...
                            if (argv[optind + 3] != nullptr) {
                                if (!StrToInt(argv[optind + 3], delayTime)) {
                                delayTime = 0 ;
                                std::cout << "No delay operation is performed" << std::endl;
                                }else {
                                std::cout << "Delay "<<delayTime<<" time one instruction" << std::endl;
