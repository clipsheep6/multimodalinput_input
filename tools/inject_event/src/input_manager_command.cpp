...
{
    constexpr int32_t ARGC_MIN = 4; // 4: min number of parameters
    constexpr int32_t TEXT_MAX_LEN = 2000; // 2000: max number of characters

    int32_t len = strlen(argv[ARGC_MIN -1]);
    if (len <= 0) {
        ...
        std::cout << TEXT_MAX_LEN << " bytes." << std::endl;
        len = TEXT_MAX_LEN;
    }

    char textChar = argv[ARGC_MIN - 1][0];
    bool isPressShift = false;
    int32_t keyCode = 0;
    for (int32_t i = 0; i < len; ++i) {
        textChar = argv[ARGC_MIN - 1][i];
